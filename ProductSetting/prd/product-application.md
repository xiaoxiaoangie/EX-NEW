...
| **SP 独立审核**          | 本期sp只有B；每个产品由其对应的 SP 独立审核，审核结果通过 API 回调通知 EX HUB                                    |
...
| 3a | **EX 租户商户 — 使用 BB 能力**  | 数币发卡，通过 EX 新系统<br />法币+数币发卡都只用BB | ✅**本期重点** |
| 3b | **EX 租户商户 — 使用 IPL 能力** | 发卡产品嵌入 原 EX 白牌系统                         | ✅**本期重点** |
| 3c | **EX 租户商户 — 使用 IPL + BB** | 本期不涉及                                          | ❌ 不涉及            |
| 4  | **API 客户**                     | 通过 API 直接对接，使用新接口                       | ✅**本期重点** |

> **本期 EX 新系统覆盖 3a 和 3c**：通过 EX 平台使用 BB 能力（数币发卡）或 IPL + BB 能力（法币+数币发卡）的租户商户。

### 2.1 Crypto Checkout 产品客户分类

| # | 客户类型                              | 使用方式                                | 本期是否涉及         |
| - | ------------------------------------- | --------------------------------------- | -------------------- |
| 1 | **BB 直客-老**                  | 信息同步到:EX 租户=BB, 开通 Crypto Checkout 产品 | ✅**本期重点** |
...
| **BB**  | 数币发卡 + Crypto Checkout 能力提供方 | 体系内 SP，本期由 EX 帮其开发系统，可接收 SumSub 数据 |

> IPL 和 BB 虽然是体系内的 SP，但在产品开通流程中作为**独立审核方**，各自审核自己负责的产品。

### 2.3 Crypto Checkout 产品

本期 Crypto Checkout 的 SP 为 **BB**。客户通过 EX 平台开通 Crypto Checkout 产品，审核由 BB 完成。
...
| 产品               | 英文名          | 说明                                               |
| ------------------ | --------------- | -------------------------------------------------- |
| **发卡**     | Card Issuing    | VCC/实体卡发行、消费管控、实时追踪                 |
| **Crypto Checkout** | Crypto Checkout | 接受 USDT/USDC 等稳定币支付，支持在线支付、Invoice |
...
| BB 直客用 BB Crypto Checkout       | BB   | BB       | BB（SP 角色）  | BB     | 租户=SP，只审核一次          |
