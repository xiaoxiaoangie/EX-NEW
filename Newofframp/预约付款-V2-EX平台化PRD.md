...
V2 同时承接 V1.1 之后的正式“1:1 余额出款”形态：客户统一余额仍由 Cactus 承载，客户下单后由 EX 冻结可用余额，按固定班车调拨至 Gate 后执行 Off + POBO。该模式依赖下单冻结、跨渠道调度和充值认领三项基础设施；V1.1 未建设这些能力，故仅以“直充绑定订单”验证 Gate 全链路。
...
6. 建设余额冻结、固定班车调度与资金认领能力，支持固定汇率 1:1 余额出款。
...
- 对客手续费计算与收取为平台内部逻辑，由独立计费产品完成，不调用 Gate 接口；Gate 仅承担渠道执行与渠道成本记录。
- 收费因子继续使用“产品=预约付款”。
- EX 产品开通记录关联计费方案。
