...
- 页面中的充值地址是平台根据客户选择的充值币种和网络，为本次预约单生成的收款工具；不是让客户从地址簿中选择平台收款地址。
- 客户实际发起转账的来源汇款地址必须提前报备并验证。页面应使用黄色提示说明报备要求，并提供“添加汇款地址”快捷入口；来源地址未验证或信息异常时不得按正常自动入账路径处理。
...
一笔预约付款以“预约主单”承载客户确认的业务意图，并在实际执行时拆分为三类可独立追踪的子单：充值子单、承兑子单、付款子单。主单与子单必须使用系统 ID 关联，不得仅依赖金额、时间或客户名称匹配。

| 层级 | 单据 | 创建时点 | 核心职责 | 与主单关系 |
| --- | --- | --- | --- | --- |
| 主单 | 预约单 | 客户锁定报价并提交后 | 固化收款人、目标法币金额、1:1 锁定汇率、费用快照、充值资产/网络及地址 | 一笔主单关联零或一笔充值子单、零或一笔承兑子单、零或一笔付款子单；异常重试需保留尝试记录 |
| 子单 1 | 充值子单 | 预约单生成充值指令时 | 跟踪预计到账、实际到账、网络、地址、TxID、确认数及充币手续费 | 必须引用原预约单号 |
| 子单 2 | 承兑子单 | 充值满足执行条件后 | 跟踪承兑卖出金额、买入金额、实际承兑汇率、承兑手续费及渠道结果 | 必须引用原预约单号和充值子单号 |
| 子单 3 | 付款子单 | 承兑成功且可付款金额确定后 | 跟踪收款人快照、收款金额、付款手续费、POBO 附加费及付款结果 | 必须引用原预约单号和承兑子单号 |

前序子单未满足执行条件时，后续子单可以显示“未创建”，不得生成伪成功记录。若渠道处理需要多次尝试，同一类型可产生多笔尝试子单，但必须标识当前有效子单、重试关系和最终结果，防止重复承兑或重复付款。

#### 11.4.2 预约付款记录列表

预约付款记录列表按预约主单展示，每行包含：预约单号、收款人、预计充值金额、锁定汇率、手续费、收款金额、状态和详情操作。

- “预计充值金额”在同一单元格展示金额、充值货币和网络。
- “锁定汇率”当前统一对客展示 1:1，并同时标识币种对，例如 `1 USDC = 1 USD`。底层实际承兑汇率在承兑子单中单独展示，不得用其追溯修改客户已锁定的 1:1 报价。
- “手续费”汇总展示承兑手续费、付款手续费和 POBO 附加费；充币手续费在详情的充值子单中展示。总费用与各子单费用必须可勾稽。
- 列表状态使用主单聚合状态，不得直接以某一个子单状态替代。

#### 11.4.3 预约付款详情

详情由三部分构成：预约单概要、主单状态时间轴、子单明细。

1. 预约单概要：原预约单号、收款人及账户快照、预计充值金额、1:1 锁定汇率、收款金额、主单状态。
2. 状态时间轴：至少覆盖预约单创建、等待/确认充值、承兑处理、付款处理和最终结果；每个节点记录事件时间、事件来源和关联子单号。
3. 子单明细：
   - 充值子单：原预约单号、充币子单号、预计到账金额、实际到账金额、充值网络、地址/TxID、充币手续费、状态。
   - 承兑子单：承兑子单号、卖出金额、买入金额、实际承兑汇率、承兑手续费、状态。
   - 付款子单：付款子单号、收款人快照、收款金额、付款手续费、POBO 附加费、状态。

主单及子单详情均使用下单/执行时的不可变快照展示。收款人、费率或渠道配置后续发生变化，不得覆盖历史订单记录。

预约付款创建页中的“充值币种 / 充值网络 / 平台生成地址”应采用紧凑布局：币种选择与网络选择分别占一行，生成地址以二维码和文本同时展示。充值地址区域与费用明细区域左右并列，减少纵向空间。页面需区分：平台生成的本次充值地址属于订单收款工具；地址簿维护的是客户实际转出资金的来源汇款地址。付款用途下方提供选填附言，并按渠道允许字符集和长度校验。
...
| reservation_order_id  | 预约付款必填 | 系统 ID    | 租户内唯一                           | 客户/内部 | 预约付款主单号                       |
| deposit_sub_order_id  | 条件必填     | 系统 ID    | 创建充值指令后唯一                   | 客户/内部 | 充值子单号，引用预约主单             |
| conversion_sub_order_id | 条件必填   | 系统 ID    | 发起承兑后唯一                       | 客户/内部 | 承兑子单号，引用主单及充值子单       |
| payout_sub_order_id   | 条件必填     | 系统 ID    | 发起付款后唯一                       | 客户/内部 | 付款子单号，引用主单及承兑子单       |
| actual_deposit_amount | 条件必填     | 金额       | 链上确认结果可追溯                   | 客户/内部 | 充值实际到账金额                     |
| actual_conversion_rate | 条件必填    | 数值       | 承兑结果可追溯                       | 客户/内部 | 与对客锁定 1:1 报价分开记录          |
...
页面交互见：[fiat-crypto-platform-interactive.html](./fiat-crypto-platform-interactive.html)

当前交互覆盖客户动态路径演示、标准/专属互斥展示、钱包付款/预约付款、1:1 锁定报价、调拨成本报价、POBO/非 POBO、预约付款记录、主单时间轴及充值/承兑/付款子单详情。
...
15. 预约付款列表按主单展示预约单号、收款人、预计充值金额及网络、1:1 锁定汇率、分项手续费、收款金额、聚合状态和详情入口。
16. 预约付款详情可从主单追踪充值、承兑、付款三类子单；子单编号、金额、实际汇率、分项费用和状态与资金流水及渠道结果一致。
17. 前序子单未成功时不会错误创建或执行后续子单；重试、重复回调和结果未知不会造成重复承兑或重复付款。
