...
### 安全验证统一规则（Security Verification）

修改/设置登录凭证、密码、交易密码、Designated Phone 等均属于**高敏感操作**，需要先验证身份。

**核心原则：有2FA → 验2FA + 设置新的；没2FA → 验当前的 + 设置新的。**

所有高敏感设置操作遵循同一套验证模式：

| 操作类型 | 有2FA | 没2FA |
| --- | --- | --- |
| **修改登录凭证（邮箱/手机）** | 验2FA → 输入新凭证 → 验证新凭证 | 验当前凭证 → 输入新凭证 → 验证新凭证 |
| **添加登录凭证（首次设置）** | 验2FA → 输入新凭证 → 验证新凭证 | 验当前凭证 → 输入新凭证 → 验证新凭证 |
| **修改密码** | 验2FA → 输入新密码 | 输入当前密码 → 输入新密码 |
| **首次设置密码** | 验2FA → 输入新密码 | 验当前凭证 → 输入新密码 |
| **修改 PIN** | 验2FA → 输入新PIN | 输入当前PIN → 输入新PIN |
| **首次设置 / 重置 PIN** | 验2FA → 输入新PIN | 验当前凭证 → 输入新PIN |
| **修改 Designated Phone** | 验2FA → 输入新手机 → 验证新手机 | 验当前凭证 → 输入新手机 → 验证新手机 |

> **2FA 优先级**：Authenticator App > SMS 2FA > Email 2FA，验证最高优先级的一个。
> **「验当前凭证」** = 向当前登录凭证（邮箱或手机号）发送验证码，或输入当前密码/PIN（视具体操作而定）。
> **为什么优先2FA？** 2FA（尤其 Authenticator App）是离线因素，无法被远程劫持。

### 6.1 修改/设置邮箱

```mermaid
flowchart TD
    A[点击 Email 的 Edit / Add 按钮] --> B{是否已开启2FA}

    B -->|已开启2FA| C[Step 1: 验证最高优先级2FA<br/>Authenticator > SMS > Email]
    B -->|未开启2FA| D{是否已有邮箱}

    D -->|已有邮箱-修改| D1[Step 1: 向原邮箱发送验证码]
    D -->|无邮箱-首次添加| D2[Step 1: 向当前手机号发送验证码]
    ...
    E --> F[Step 2: 输入新邮箱地址]
    F --> G[向新邮箱发送验证码]
    G --> H[Step 3: 输入新邮箱收到的验证码]
    H --> I{验证码正确}

    I -->|正确| J[邮箱修改/设置成功<br/>新邮箱状态: Verified]
    I -->|错误| H1[提示验证码错误]
```
...
```mermaid
flowchart TD
    A[点击 Mobile 的 Edit / Add 按钮] --> B{是否已开启2FA}

    B -->|已开启2FA| C[Step 1: 验证最高优先级2FA<br/>Authenticator > SMS > Email]
    B -->|未开启2FA| D{是否已有手机号}

    D -->|已有手机号-修改| D1[Step 1: 向原手机号发送验证码]
    D -->|无手机号-首次添加| D2[Step 1: 向当前邮箱发送验证码]
    ...
    F --> G[向新手机号发送验证码]
    G --> H[Step 3: 输入新手机号收到的验证码]
    H --> I{验证码正确}

    I -->|正确| J[手机号修改/设置成功<br/>新手机号状态: Verified]
    I -->|错误| H1[提示验证码错误]
```
...
```mermaid
flowchart TD
    A[点击 Password 的 Change / Set up 按钮] --> B{是否已开启2FA}

    B -->|已开启2FA| C[Step 1: 验证最高优先级2FA<br/>Authenticator > SMS > Email]
    B -->|未开启2FA| D{是否已设置密码}

    D -->|已设置-修改| D1[Step 1: 输入当前密码]
    D -->|未设置-首次设置| D2[Step 1: 向登录凭证发送验证码]

    C --> G[Step 2: 输入新密码]
    D1 --> G
    D2 --> G

    G --> H[实时校验密码强度<br/>大于等于8位 大写 小写 数字/符号]
    H --> I[确认新密码 二次输入]
    I --> J{两次密码一致}
...
```mermaid
flowchart TD
    A[Settings - Security - Payment PIN] --> B{是否已开启2FA}

    B -->|已开启2FA| C[Step 1: 验证最高优先级2FA<br/>Authenticator > SMS > Email]
    B -->|未开启2FA| D{当前PIN状态}

    D -->|已设置-修改| D1[Step 1: 输入当前PIN]
    D -->|未设置或重置| D2[Step 1: 向登录凭证发送验证码]
...
```mermaid
flowchart TD
    A[Settings - Security - Designated Phone<br/>仅 Account Holder 可见] --> B{是否已开启2FA}

    B -->|已开启2FA| C[Step 1: 验证最高优先级2FA<br/>Authenticator > SMS > Email]
    B -->|未开启2FA| D[Step 1: 向登录凭证发送验证码]

    C --> F[Step 2: 输入指定手机号<br/>选择区号 + 输入号码]
    D --> F

    F --> G[向该手机号发送验证码]
    G --> H[Step 3: 输入该手机号收到的验证码]
    H --> I{验证通过}
...
### 7.9 2FA 与 Payment PIN 使用场景总结

**2FA 定位：强身份验证** — 证明「你就是你」，用于安全敏感操作。
**PIN 定位：快捷交易确认** — 确认操作意图，用于日常资金操作，替代每次发送验证码。

| 操作场景                                   | 有2FA时                    | 没2FA时                          |
| ------------------------------------------ | -------------------------- | -------------------------------- |
| **登录（密码登录的第二因子）**       | 验2FA                      | 直接登录成功                     |
| **修改登录凭证（邮箱/手机号）**      | 验2FA → 新凭证 → 验新凭证 | 验当前凭证 → 新凭证 → 验新凭证  |
| **添加登录凭证（首次设置）**         | 验2FA → 新凭证 → 验新凭证 | 验当前凭证 → 新凭证 → 验新凭证  |
| **修改登录密码**                     | 验2FA → 新密码             | 当前密码 → 新密码                |
| **首次设置密码**                     | 验2FA → 新密码             | 验当前凭证 → 新密码             |
| **修改 Payment PIN**                 | 验2FA → 新PIN              | 当前PIN → 新PIN                  |
| **首次设置/重置 PIN**                | 验2FA → 新PIN              | 验当前凭证 → 新PIN              |
| **设置/修改 Designated Phone**       | 验2FA → 新手机 → 验新手机 | 验当前凭证 → 新手机 → 验新手机  |
| **开启/关闭 2FA**                    | 验2FA                      | 验当前凭证                       |
| **API Key 创建/删除**                | 验2FA                      | 验当前凭证                       |
| **日常资金操作（转账/付款/提现）**   | PIN 优先，PIN 未设置时验证码 | PIN 优先，PIN 未设置时验证码   |
| **PIN 锁定后的资金操作**             | 降级到验证码               | 降级到验证码                     |

**核心原则：有2FA → 验2FA + 设置新的；没2FA → 验当前的 + 设置新的。做交易用 PIN。**

> **注意：** 「验当前凭证」= 向登录凭证（邮箱/手机）发送验证码。「当前密码/PIN」= 直接输入已有的密码或PIN。日常资金操作不走2FA，走 PIN 或验证码。
...
| N04  | 2FA 验证码（SMS）     | 短信      | 密码登录触发 SMS 2FA，或安全设置变更时验证身份    |
| N05  | 2FA 验证码（Email）   | 邮件      | 密码登录触发 Email 2FA，或安全设置变更时验证身份  |
...
| N20  | Payment PIN 修改成功  | 邮件      | PIN 修改/设置完成后确认                                 |
| N21  | 安全设置变更验证码    | 邮件/短信 | 修改密码/PIN/凭证/Designated Phone 时的身份验证码       |
...
> 在以下场景发送：① **密码登录**时触发 SMS 2FA（验证码/OTP 登录不发）；② **安全设置变更**（修改密码/PIN/凭证/Designated Phone 等）时作为身份验证。
...
> 在以下场景发送：① **密码登录**时触发 Email 2FA（验证码/OTP 登录不发）；② **安全设置变更**（修改密码/PIN/凭证/Designated Phone 等）时作为身份验证。
...
#### N20 Payment PIN 修改成功

| 语言     | 邮件标题                             | 邮件正文摘要                                                                                                                                           |
| -------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| EN       | Your Payment PIN has been changed    | Your Payment PIN was successfully changed on {datetime}. If this wasn't you, please reset your PIN immediately and contact support.                    |
| 简体中文 | 您的交易密码已修改                   | 您的交易密码已于 {datetime} 成功修改。如非本人操作，请立即重置交易密码并联系客服。                                                                      |
| 繁體中文 | 您的交易密碼已更改                   | 您的交易密碼已於 {datetime} 成功更改。如非本人操作，請即刻重設交易密碼並聯絡客服。                                                                      |

#### N21 安全设置变更验证码

> 用户修改密码/PIN/登录凭证/Designated Phone 等安全设置时，向登录凭证发送的身份验证码（非2FA用户场景）。

| 语言     | 短信内容                                                                                                      | 邮件标题                          |
| -------- | ------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| EN       | Your security verification code is {code}. Valid for 5 minutes. If you didn't request this, ignore this message. | Security verification code        |
| 简体中文 | 您的安全验证码是 {code}，5分钟内有效。如非本人操作请忽略。                                                    | 安全验证码                        |
| 繁體中文 | 您的安全驗證碼係 {code}，5分鐘內有效。如非本人操作請忽略。                                                    | 安全驗證碼                        |
