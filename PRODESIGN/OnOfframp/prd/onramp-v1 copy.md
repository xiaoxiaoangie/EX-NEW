...
5. [业务流程与时序图](#5-业务流程与时序图)
   ...
   - 5.4 [场景 B1：预约单 — BB 收款（代收付/XPAY VA）→承兑（仅 BB）](#54-场景-b1预约单--bb-收款代收付xpay-va承兑仅-bb)
...
| 范围        | 说明                                         |
| ----------- | -------------------------------------------- |
| ✅ 本期包含 | OnRamp（买入数币：法币→数币）A1 + B1 场景  |
| ❌ 本期不含 | A2 场景：IPL 法币跨 SP 划转到 BB 承兑（Phase 2） |
| ❌ 本期不含 | B2 场景：IPL VA 收款+跨 SP 划转+BB 承兑（Phase 2）|
...
| ---------------- | ------------------------------------ | -------------------------------------------------------------------------------------------- |
| 场景 A2          | IPL 法币直接通过中间户承兑到 BB 数币 | **Phase 2**：跨SP划转违反账户封闭原则，本期不做                         |
| 场景 B2          | IPL VA 收款→中间户→BB 承兑         | **Phase 2**：涉及跨SP划转，本期不做 |
| IPL→BB 资金划转 | 中间户内部记账，无独立风控           | **Phase 2**：独立交易单，走风控/三单/计费                                                 |
...
```
规则1（单 SP 承兑）：所有买入数币（OnRamp）承兑必须从 BB 法币账户发起
       → 不存在“从 IPL 法币直接买入数币”

规则2（账户封闭）：OnRamp 流程中不跨 SP 划转，承兑资金必须已在 BB 承兑法币账户内
       → IPL 收付款账户 → BB 承兑法币账户 是独立操作，不属于 OnRamp 范畴

规则3（跨账户划转）：IPL→BB 的资金划转 = Phase 2（独立交易单，走风控/计费）
       → 如商户需要将 IPL 法币转到 BB 承兑账户，应通过独立的账户间划转操作完成
...
场景 A2（IPL→BB，即时单）⚠️ Phase 2：
  IPL 收付款账户 ──转账(风控/计费)──→ BB 承兑法币账户 ──承兑──→ BB 数币钱包

场景 B1（仅 BB，预约单）：
  外部汇款人 ──BB代收付账户──→ BB 承兑法币账户 ──承兑──→ BB 数币钱包
  外部汇款人 ──XPAY VA────→ BB 承兑法币账户 ──承兑──→ BB 数币钱包
  （BB 代收付 和 XPAY VA 是 BB 内部两种收款渠道，对商户透明）

场景 B2（IPL VA，预约单）⚠️ Phase 2：
...
| A2   | BB+IPL | IPL 法币余额     | 1      | 2（IPL→BB 转账 + BB 承兑）            | 0           | 商户即时发起    | ⚠️ **Phase 2** |
| B1   | 仅 BB  | 银行转账(BB代收付/XPAY VA) | 1      | 2（BB 收款 + BB 承兑）                 | 1（BB PP）  | 预约单+渠道触发 |
| B2   | BB+IPL | 银行转账(IPL VA) | 1      | 3（IPL 收款 + IPL→BB 转账 + BB 承兑） | 1（IPL PP） | 预约单+渠道触发 | ⚠️ **Phase 2** |
...
> **设计原则**：承兑资金必须已在 BB 承兑交易账户内，不允许在 OnRamp 流程中从收付款账户（IPL）跨 SP 划转到承兑账户（BB）。
> 如商户需要将 IPL 法币转到 BB 承兑账户，应通过**独立的账户间划转**操作完成，不属于 OnRamp 范畴。
...
### 5.3 场景 A2：即时单 — IPL 收付款账户→BB 承兑账户→上链【Phase 2 — 本期不做】

> 🚧 **Phase 2 场景**：本场景涉及从收付款账户（IPL）跨 SP 划转到承兑账户（BB），违反“承兑资金必须已在承兑交易账户”的设计原则，放入下一期。如商户需要将 IPL 法币转到 BB 承兑账户，应通过独立的账户间划转操作完成。
>
...
### 5.4 场景 B1：预约单 — BB 收款（代收付/XPAY VA）→承兑（仅 BB）

**场景：** 外部汇款人通过 BB 收款渠道（BB 代收付账户 或 XPAY VA）汇入 USD，收款到 BB 承兑法币账户后，承兑为 USDT 到 BB 数币钱包。

**特点：** 收款（渠道触发）+ 承兑，2 笔交易单串行。BB 内部决定使用代收付账户还是 XPAY VA 渠道，对商户透明。

**BB 内部收款渠道：**

| 渠道 | 说明 | 资金流向 | 商户感知 |
| ---- | ---- | -------- | -------- |
| BB 代收付账户 | BB 自有代收付能力 | 外部汇款 → BB 代收付账户 → BB 承兑法币账户 | 商户看到"银行转账收款" |
| XPAY VA | BB 内部 Channel 提供的 VA 能力 | 外部汇款 → XPAY VA → BB 承兑法币账户 | 商户看到"同名 VA 账户"，不感知 XPAY |

> ⚠️ 商户下预约单后，系统根据 BB 内部渠道配置展示对应的收款账户信息（代收付账户或 VA 账户）。底层渠道对商户/TP 透明。

**单据结构（2 笔交易单）：**

```
商户单 M001 (OnRamp: BB 收款→USD→USDT)  ← 仅 BB
    ├── 交易单 T001 (BB): 收款 — 外部 USD 入到 BB 承兑法币账户（渠道通知触发）
    │       └── 渠道单 C001 (BB PP): BB 代收付 或 XPAY VA 渠道执行记录
    └── 交易单 T002 (BB): 承兑 — BB USD→USDT

单据数：1 商户单 + 2 交易单 + 1 渠道单（仅 BB PP 可见）
资金流：外部汇款人 →[BB代收付/XPAY VA]→ 商户 BB 承兑法币账户 → 商户 BB USDT 钱包
...
sequenceDiagram
    ...
    Note over Channel,WP: 商户已下预约单（买入数币: USD→USDT）<br/>收款渠道: BB代收付 或 XPAY VA（BB内部决定，商户透明）

    rect rgb(240, 255, 240)
        Note over Channel,WP: 阶段1：T001 BB收款（BB代收付/XPAY VA 渠道通知触发）
        Channel->>CO: 1. 入账通知(USD) [BB代收付 或 XPAY VA]
...
- **T001（BB 收款）**：渠道通知触发（BB 代收付 或 XPAY VA），外部 USD 入到商户 BB 承兑法币账户，走 BB 收款风控（汇款人 AML），有渠道单
- **T002（BB 承兑）**：BB USD→USDT，内部账户划转，走 BB 承兑风控
- **预约单模式**：商户先下预约单，收款到账后自动触发 T002 承兑
- **T001→T002 串行**：收款完成后才能承兑
- **计费**：收款手续费 + 承兑费用分别计算
- **渠道透明**：无论是 BB 代收付还是 XPAY VA 入账，对商户/TP 均表现为"BB 收款"，底层渠道不暴露
...
### 5.5 场景 B2：预约单 — IPL VA 收款→转入 BB 承兑账户→上链【Phase 2 — 本期不做】

> 🚧 **Phase 2 场景**：本场景涉及 IPL VA 收款后跨 SP 划转到 BB 承兑账户，复杂度最高，放入下一期。本期仅做 A1 + B1（纯 BB 内部）。
>
...
```
商户选择"银行转账"后，系统根据 BB 内部渠道配置展示收款方式：

情况1：BB 仅有代收付账户（无 XPAY VA）：
→ 直接展示 BB 代收付账户信息（无需选择）

情况2：BB 已配置 XPAY VA 渠道：
→ 展示两个选项：
   ├── 银行转账收款（BB 代收付账户，适合所有汇款人）
   └── 同名 VA 账户（XPAY VA，专属虚拟账户，按银行区分）
       ├── VA 1: DBS Bank (Hong Kong)
       ├── VA 2: OCBC Bank (Singapore)
       └── VA N: ...

⚠️ 以上均为 BB 内部渠道（Phase 1），商户不感知 XPAY，统一展示为"同名 VA 账户"

Phase 2 扩展（BB + IPL 配置）：
→ 额外展示 IPL VA 选项（跨 SP，涉及划转）
...
| 维度               | A1：BB 直接承兑     | A2：IPL→BB→承兑 ⚠️ Phase 2 | B1：BB 收款（代收付/XPAY VA）→承兑 | B2：IPL VA 收款→BB→承兑 ⚠️ Phase 2 |
| ------------------ | ------------------- | ------------------------------ | ----------------------------------- | ------------------------------------ |
| **本期范围** | ✅ Phase 1           | ❌ **Phase 2（本期不做）**    | ✅ Phase 1                         | ❌ **Phase 2（本期不做）**           |
| **触发方式** | 即时单（商户主动）  | 即时单（商户主动）             | 预约单（渠道入账触发）              | 预约单（渠道入账触发）               |
| **资金来源** | BB 承兑法币账户余额 | IPL 收付款法币账户余额         | 外部汇款（BB 代收付/XPAY VA）      | 外部汇款（IPL VA）                   |
...
| 出款风控 | IPL 收付款账户出款（Phase 2） | 商户 AML、交易频率、金额合规     | 解冻/拒绝                  |
| 入款风控 | BB 承兑法币账户入款（Phase 2） | 来源合规性、商户风险等级         | 拒绝入账                   |
...
| R007     | 汇款人同名校验    | ~~B2（IPL VA）~~ Phase 2 | 汇款人 = VA 持有人（同名收款） | 匹配失败，资金挂起 |
| R008     | 交易频率          | ~~A2/B2 划转~~ Phase 2 | 划转频率≤配置上限（防洗钱）   | 拒绝划转           |
...
| T001 成功+T002 承兑失败（A2）       | A2 ⚠️ Phase 2 | 逆向回滚内部交易单 | 商户 IPL 收付款账户  |
| T002 成功+T003 承兑失败（B2）       | B2 ⚠️ Phase 2 | 逆向回滚内部交易单 | 商户 IPL 收付款账户  |
...
```
退款分类：
├── 内部逆向回滚（T001成功+T002失败 → BB承兑法币→IPL收付款）Phase 2
│   └── 系统自动执行，商户不可见，不收承兑费
│
...
| 同名校验失败（仅 B2，Phase 2） | 入账但冻结，合规审核               | 通知交易待审核                |
...
### 10.3 渠道单状机（B1：BB代收付/XPAY VA；B2 Phase 2）
...
| **IPL→BB 资金划转** | 中间户内部划转，无独立风控          | Phase 2：独立交易单，走风控/三单/计费                             |
| **A2 场景**  | 1 个商户单 + 2 个交易单（简单划转） | Phase 2：跨SP划转违反账户封闭原则，本期不做                     |
| **B2 场景**  | 1 个商户单 + 2 个交易单             | Phase 2：涉及跨SP划转，本期不做                 |
| **划转风控**         | 无独立风控                          | Phase 2：IPL 出款风控 + BB 入款风控                                 |
| **划转计费**         | 无独立计费                          | Phase 2：有划转手续费                                                       |
| **跨 SP 异常处理**   | 简单回滚                            | Phase 2：逆向回滚，创建内部退款交易单 RT |
...
| 1.1  | 2026-02-27 | A2/B2 均推迟到 Phase 2；本期范围 = A1 + B1（纯BB内部）；承兑资金必须已在承兑交易账户，不跨SP |
