...
- ✅ **安全优先**：密码登录支持 2FA 双因素认证；验证码/OTP 登录本身即为强因素验证，不再叠加 2FA
...
EX 平台支持两种注册方式，**系统根据用户 IP 地区自动选择默认凭证类型和注册方式**，用户无需手动选择：

| 地区 | 默认凭证类型 | 默认注册方式 | 说明 |
| ---- | -------- | -------- | ---- |
| 中国大陆 | 手机号（+86） | 密码注册 | 手机 + 密码 + 验证码（一步完成）|
| 香港 | 手机号（+852） | 密码注册 | 同上 |
| 其他国家/地区 | 邮箱 | 验证码注册 | 邮箱 + 验证码，验证通过即注册 |

用户可手动切换凭证类型（邮箱 ↔ 手机号），切换后注册方式自动联动：手机号 → 密码注册，邮箱 → 验证码注册。用户也可在页面上手动切换注册方式。

**两种注册方式详情：**

- **密码注册**：输入手机号/邮箱 + 设置密码 + 验证码（同一页面内内联输入）→ 点击注册即完成
- **验证码注册**：输入手机号/邮箱 + 6 位验证码（同一页面内内联输入）→ 验证通过即完成注册（登录即注册）
...
flowchart TD
    ...
    B --> C{地区判断}
    C -->|中国大陆/香港| D1[默认显示手机号Tab<br/>自动填充区号 +86/+852<br/>默认密码注册]
    C -->|其他国家| D2[默认显示邮箱Tab<br/>默认验证码注册]

    D1 --> E[输入手机号/邮箱]
    D2 --> E

    E --> F{注册方式}
    F -->|密码注册| G1[同一页面内联填写：<br/>密码 + 确认密码 + 6位验证码]
    F -->|验证码注册| G2[同一页面内联填写：<br/>6位验证码]

    G1 --> H[点击「注册」按钮]
    G2 --> H

    H --> I{凭证验证}
    I -->|验证通过| OK[注册成功<br/>创建 Identity ID]
    I -->|验证失败| J[提示错误，可重试]
...
flowchart TD
    ...
    C -->|其他国家/识别失败| D3[默认邮箱Tab<br/>注册方式: 验证码注册]
    D1 --> E[用户可手动切换凭证类型]
    D2 --> E
    D3 --> E
    E --> F[切换后注册方式自动联动<br/>手机号→密码 / 邮箱→验证码]
```
...
若账户同时绑定了密码，也可选择验证码登录（登录即注册路径）。

**2FA 触发规则（简化原则）：**

| 登录方式 | 触发 2FA | 原因 |
| -------- | -------- | ---- |
| 密码登录 | ✅ 触发（如已开启） | 密码属于「你知道的」，2FA 提供第二因素「你拥有的」 |
| 验证码登录（SMS/Email） | ❌ 不触发 | 验证码本身是一次性强因素，安全强度已够，叠加同渠道 2FA 无意义 |
| OTP 登录（Authenticator） | ❌ 不触发 | TOTP 本身即强因素（你拥有），再叠加同类 2FA 冗余 |
...
flowchart TD
    ...
    H2a -->|是| H2c[要求输入2FA验证码<br/>仅密码登录触发]
    H2a -->|否| J1
    H2c --> H2d{2FA验证通过}
    H2d -->|是| J1
    H2d -->|否| H2e[提示2FA验证失败]

    note1["验证码登录 / OTP登录<br/>本身已是强因素，不触发2FA"]
```
...
flowchart TD
    ...
    I -->|未开启| K[登录成功]
    I -->|已开启| L[输入 2FA 验证码<br/>仅密码登录路径]
    L -->|通过| K
    L -->|失败| L1[提示 2FA 验证失败]

    note2["⚠️ 验证码登录 / OTP 登录直接进入 K<br/>不经过 2FA 步骤"]
...
> **核心原则：2FA 仅在密码登录时触发。** 验证码登录（SMS/Email OTP）和 Authenticator OTP 登录本身已是强因素验证，不叠加 2FA，避免同渠道重复验证。

```mermaid
flowchart TD
    A[用户选择登录方式] --> B{登录方式}

    B -->|验证码登录| C1[输入邮箱/手机 + 6位验证码]
    B -->|OTP 登录| C2[输入邮箱/手机 + Authenticator 动态码]
    B -->|密码登录| C3[输入邮箱/手机 + 密码]
    ...
    E -->|未开启| OK
    E -->|已开启| F{Default 2FA 方式}

    F -->|Authenticator App| F1[要求输入 Authenticator 动态码]
    F -->|SMS| F2[自动发送短信验证码到绑定手机号]
    F -->|Email| F3[自动发送邮件验证码到绑定邮箱]
...
| 登录方式 | 原因 |
| -------- | ---- |
| 验证码登录（SMS） | SMS OTP 本身已是一次性强因素，再叠加 SMS 2FA = 同渠道重复验证 |
| 验证码登录（Email） | 同上，Email OTP 即强因素 |
| OTP 登录（Authenticator） | TOTP 本身是「你拥有的」强因素，与 Authenticator 2FA 同类，叠加冗余 |
...
| N04  | 2FA 验证码（SMS）     | 短信      | **密码登录**触发 SMS 2FA（验证码/OTP 登录不发） |
| N05  | 2FA 验证码（Email）   | 邮件      | **密码登录**触发 Email 2FA（验证码/OTP 登录不发） |
...
| N18  | Payment PIN 锁定通知    | 邮件      | PIN 连续错误 5 次锁定 30 分钟               |
| N19  | 新设备 OTP 登录通知  | 邮件      | OTP 登录方式检测到新设备登录成功           |
...
> 仅在**密码登录**并已开启 SMS 2FA 时发送。验证码登录 / OTP 登录不发此通知。

| 语言     | 短信内容                                                     |
| -------- | ------------------------------------------------------------ |
| EN       | {code} is your 2FA code for {platform}. Valid for 5 minutes. |
| 简体中文 | {code} 是您的 {platform} 二次验证码，5分钟内有效。           |
| 繁體中文 | {code} 係您的 {platform} 二次验证码，5分钟內有效。           |

#### N05 2FA 验证码（Email）

> 仅在**密码登录**并已开启 Email 2FA 时发送。验证码登录 / OTP 登录不发此通知。
...
| 繁體中文 | 您的二次验证码                      | 您的二次验证码係 {code}，5分钟內有效。如非本人登入，請即刻更改密碼。                                                                |
...
#### N18 Payment PIN 锁定通知

| 语言     | 邮件标题                        | 邮件正文摘要                                                                                                                  |
| -------- | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| EN       | Your Payment PIN has been locked | Your Payment PIN has been temporarily locked for 30 minutes due to 5 consecutive incorrect attempts. You can use SMS/Email verification instead during this period. |
| 简体中文 | 您的交易密码已被锁定         | 您的交易密码因连续 5 次输入错误已被临时锁定 30 分钟。锁定期间可使用验证码方式继续操作。                |
| 繁體中文 | 您的交易密码已被鎖定         | 您的交易密码因連續 5 次輸入錯誤已被臨時鎖定 30 分鐘。鎖定期間可使用验證碼方式繼續操作。                |

#### N19 新设备 OTP 登录通知

> OTP 登录方式（Authenticator）检测到新设备登录成功时发送，与 N13（密码/验证码登录新设备）内容相同，区分登录方式共用模板。

| 语言     | 邮件标题                        | 邮件正文摘要                                                                                                                                               |
| -------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| EN       | New device login detected (OTP) | A new login using OTP was detected on {datetime} from {device} ({ip}, {location}). If this wasn't you, please disable your Authenticator App and contact support immediately. |
| 简体中文 | 检测到新设备 OTP 登录     | 您的账户于 {datetime} 在新设备上通过 OTP 登录：{device}（{ip}，{location}）。如非本人操作，请立即禁用 Authenticator App 并联系客服。 |
| 繁體中文 | 偉測到新裝置 OTP 登入     | 您的帳戶於 {datetime} 在新裝置上透過 OTP 登入：{device}（{ip}，{location}）。如非本人操作，請即刻禁用 Authenticator App 並聯絡客服。 |
...
| `{lock_duration}` | 锁定时长 | 30 minutes / 30 分钟 |
