...
## 二·补充、跨 SP 转账的落地模式（有 VA / 无 VA）

> 结汇本质是一次**跨 SP 转账**：资金从源 SP（如 BB 的 CNH）转到结汇 SP（鲲鹏的 CNH）。不同 SP 与 EX 的关系不同、落地方式也不同，但对客户**统一包装成「跨 SP 转账」这一个动作**，落地方式由 EX 路由按目标 SP 属性自动选择，客户无感。

### 客户视角：统一的「跨 SP 转账」

- 客户只看到：**我的 CNH 余额从「BB 承载」转到「结汇机构承载」**，一步操作，不感知底层是 VA(待确认)、大账户还是内部调拨。
- **不包装成"跨机构转账"**：那样产品无特色、体验差（等于"A 机构申请 VA、B 机构付款"）；统一为「跨 SP 转账」，VA / 大账户 / 内部调拨都是背后管道。

### 三种落地方式（按目标 SP 属性）

| 落地方式                           | 代表         | 前提                           | 说明                                                                                                  | 归属 / 对账                                |
| ---------------------------------- | ------------ | ------------------------------ | ----------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| **VA 收款（有 VA 模式）**    | 宝付（举例） | 目标 SP 支持为客户开专属 VA    | 客户**首次转账时申请一个专属 VA，此后每次跨 SP 转账默认走该 VA**；源 SP 一笔出款 → VA 一笔入款 | VA 与客户 1:1 绑定，无需备注匹配，归属清晰 |
| **大账户收款（无 VA 模式）** | 鲲鹏（当前） | 目标 SP 仅提供一个大额收款账户 | 客户付款进结汇机构大账户，需带标识（申报人 / 订单号 / 明细），由结汇机构按明细匹配解付（见 UC-06）    | 依赖明细 / 备注匹配，故需"上传明细→解付"  |

### 设计结论：用「跨 SP 转账」抽象，VA 作为默认管道

- **有 VA 的 SP**：客户**首次转账时申请 VA，之后默认每次都走该 VA**（首次触发申请、后续复用，机制上与 UC-13"首次申报人报备、后续复用"一致）。
  - ⚠️ **待办（VA 客户感知 / 合规）**：不宜完全黑盒"自动"替客户开 VA。VA 是客户 / 收款人名下账户，**首次应让客户显式知情并授权**（弹出说明 + 授权签署）、VA 号与归属对客可见；否则可能涉及**代客开户授权 / KYC 责任**风险，且结汇 SP 事后**调单**时客户不知情、缺乏可解释性。详见 §八 8.2。
- **无 VA 的 SP（鲲鹏当前）**：走大账户收款 + 明细匹配（UC-06 解付）；后续鲲鹏若支持 VA，可平滑升级为 VA 模式，**客户无感**。
- **交易记录模型**：无论哪种落地方式，一次跨 SP 转账 = **1 笔商户单 + 2 笔交易单**（源 SP 出款 + 目标 SP 入款，详见 UC-05）。

**SP 调拨关系对配置示例**

| 源 SP | 目标 SP | 方向 | 币种 | 落地模式   | 状态 |
| ----- | ------- | ---- | ---- | ---------- | ---- |
| BB    | 鲲鹏    | 单向 | CNH  | 大账户收款 | 启用 |
| BB    | 宝付    | 双向 | CNH  | VA 收款    | 启用 |

> 说明：币种列取源 SP 与目标 SP 各自支持可调拨币种的交集；本期不列 A（IPL）。

### 关键区分：「同租户跨 SP 调拨」 vs 「跨租户真实收付」

> 两个场景表面都涉及"资金从一个 SP 到另一个 SP"，但**本质完全不同**，产品处理方式也不同。判定的**唯一分水岭 = 客户是否通过同一个租户进来（是否同一主体在同一租户下）**。

**场景一：同租户跨 SP —— 走「资金调拨」（本方案 UC-05 / UC-14）**

- **前提**：**同一个客户、通过同一个租户进来**，其名下同时挂了多个 SP（如租户下 SP=BB、SP=鲲鹏）。
- **本质**：EX 内部编排，把该客户在 SP-A 的余额搬到 SP-B，**对客户是"我自己的一笔资金调拨"**。
- **记账**：1 商户单 + 2 交易单（源 SP 出、目标 SP 入），属于**同一主体**的内部腾挪。
- **对客呈现**：统一为「资金调拨」，落地模式（内部调拨 / VA / 大账户）透明。

**场景二：跨租户 —— 是「真实的一笔收 + 一笔付」，不是调拨**

- **前提**：**不同租户下的不同主体**。例如客户在 A（后面 SP=A）注册、又在 B（后面 SP=B）注册，是**两个独立入网主体 / 两个收款关系**。
- **典型链路**：客户在 **A 申请 VA → 把该 VA 作为收款人绑定到 B → 在 B 发起提现 / 付款 → 资金真实打到 A 的 VA**。
- **本质**：这是 **B 的一笔真实付款（Payout）+ A 的一笔真实收款（Payin）**，两笔分属**不同租户 / 不同主体**，通过**收款人（VA）绑定**发生真实资金往来，**不是同一主体的内部调拨**。
- **不能包装成「资金调拨」**：因为不满足"同一客户、同一租户"前提；调拨产品**不覆盖**此场景。

**对比表**

| 维度     | 同租户跨 SP（资金调拨）             | 跨租户（真实收付）                            |
| -------- | ----------------------------------- | --------------------------------------------- |
| 客户入口 | **同一客户、同一租户**        | **不同租户下的不同主体**                |
| 资金性质 | 同一主体内部腾挪                    | 两个主体间真实收 + 付                         |
| 触发方式 | 客户发起「资金调拨」                | B 侧付款（Payout）＋ A 侧收款（Payin）        |
| VA 角色  | 落地管道（对客透明，可选）          | **收款人**（A 的 VA 被绑为 B 的收款人） |
| 记账     | 1 商户单 + 2 交易单（同主体）       | 两笔独立交易单，分属不同租户 / 主体           |
| 产品归属 | 「资金调拨」基础产品（UC-05/UC-14） | 常规收款（Payin）+ 付款（Payout）产品         |
| 对客叙事 | "我自己的一笔调拨"                  | "我向某收款人付款" / "我收到一笔款"           |

> **一句话**：**跨 SP ≠ 跨租户**。客户经同一租户进来、同一主体名下多 SP → 调拨；不同租户 / 不同主体经收款人绑定发生的往来 → 真实收付，不是调拨。
...
> 用例编号：UC-01 ~ UC-14；每个用例的流程用流程图（flowchart）表示。
...
> **订单与交易记录模型**：一次跨 SP 转账 = **1 笔商户单 + 2 笔交易单**——源 SP 一笔**出款**（如 BB CNH 出）、目标 SP 一笔**入款**（如鲲鹏 CNH 入）。两笔交易单同属一个商户单（transferId），分别对应两个 SP 的资金动作，各自独立记账与对账。
> **落地模式**：鲲鹏当前为**大账户收款（无 VA）模式**，需配合上传明细解付（UC-06）；若目标 SP 支持 VA，则**首次转账申请 VA、后续默认每次走该 VA**；若与源 SP 同体系则走内部调拨（见 §二·补充）。
...
4. 资金经 BB 付款渠道 → 鲲鹏划付。
5. 鲲鹏收汇到账后给 EX 发到账通知。
6. EX 生成 **1 笔商户单 + 2 笔交易单**（源 SP 出款：BB CNH 减；目标 SP 入款：鲲鹏 CNH 增），两笔交易单归属同一商户单，幂等记账。
...
- BR7（正向）一笔跨 SP 转账生成 **1 笔商户单 + 2 笔交易单**（源 SP 出款 / 目标 SP 入款），两笔交易单归属同一商户单（transferId），各自独立记账与对账。
- BR8（正向）落地模式按目标 SP 属性选择：**内部调拨**（与源 SP 同体系）/ **VA 收款**（有 VA：首次申请 VA、后续默认复用）/ **大账户收款**（无 VA：明细匹配，鲲鹏当前），对客户统一呈现为「跨 SP 转账」。
...
**交互要求**：以「跨 SP 转账」呈现，展示 **1 笔商户单 + 2 笔交易单（源 SP 出款 / 目标 SP 入款）**、收汇金额、来源（BB）/ 目标（鲲鹏）、状态（处理中 / 到账 / 失败 / 退回）；首次转账引导申报人报备（UC-13）；如鲲鹏调单，提示补单据（订单明细上传 / 其他资料线下提供）。

**验收标准**

- AC1 到账通知后鲲鹏 CNH 增、BB CNH 减、金额一致。
- AC2 失败可退回 BB CNH。
- AC3 记账幂等不重复；1 商户单对应 2 交易单（出款 / 入款），两笔各自可对账、金额一致。
...
flowchart TD
    ...
    c5C --> c5D{鲲鹏是否到账}
    c5D -->|到账| c5E[生成 1 商户单+2 交易单·BB 出款/鲲鹏 入款·幂等记账]
    c5E --> c5F[收汇成功]
...
> **背景差异**：以前 A/B 在 EX 体系内，计费由 EX 直接在共享账本上对客户落地（客户的计费同步到 A/B 即可）。现在**鲲鹏只给 EX 一个底价**、且鲲鹏是客户 CNH 账户的**真实记账方**，所以**租户对商户计费后，必须把「对客费额」同步给鲲鹏**。

- **不同步的后果**：鲲鹏只按底价实扣 → 鲲鹏实际给客户入账 / 解付的金额 **≠** EX 给客户展示的金额（EX 镜像与鲲鹏真实账本发散、对账失败）。
- **做法**：交易发生时，EX 按**对客费率**算出**对客费额**并**随交易同步给鲲鹏**；鲲鹏按对客口径实扣，**鲲鹏实际入账 = EX 对客展示**；差额（对客价 − 底价 = EX 毛利 + 租户毛利）由鲲鹏按周期结算返给 EX / 租户（走 UC-10 分佣返点）。
- **适用范围**：**体系外 SP（鲲鹏）必须外部同步对客费额**；体系内 SP（A/B）由 EX 直接在账本落地，无需外部同步。
...
5. **（体系外 SP）将对客费额随交易同步给鲲鹏**：鲲鹏按对客口径实扣 / 实际入账，保证鲲鹏实际入账 = EX 对客展示；差额（对客价 − 底价）按 UC-10 回结算给 EX / 租户。（体系内 SP 由 EX 直接在账本落地，无需本步）
6. 计提费用，落账单与流水，返回交易结果与费用明细。
...
- BR5（反向）变更不溯及存量。
- BR6（正向·新增）**对客计费结果（对客费额）须同步给实际记账的体系外 SP（鲲鹏）**，SP 按对客口径实扣，保证 **SP 实际入账 = EX 对客展示**；差额按 UC-10 返给 EX / 租户。
- BR7（约束）体系内 SP（A/B）由 EX 直接在共享账本落地，无需外部同步；体系外 SP（鲲鹏）必须外部同步对客费额。
...
| bizType       | 换汇 / 调拨 / 结汇                         | 是   |
| effectiveAt   | 生效时间                                   | 是   |
| custFeeAmount | 对客费额（同步给 SP 实扣）                 | 是   |
| baseFeeAmount | 底价费额（成本）                           | 是   |
| feeSyncStatus | 对客费额同步 SP 状态（体系外 SP 适用）     | 否   |
...
- AC5 体系外 SP（鲲鹏）：对客费额已同步，**鲲鹏实际入账 = EX 对客展示**，差额按 UC-10 回结算。

```mermaid
sequenceDiagram
    autonumber
    participant SP as SP或运营
    participant TP as 租户
    participant CUST as 客户
    participant EX as EX计费引擎
    participant KUN as 鲲鹏体系外SP
    participant AB as A或B体系内SP

    SP->>EX: 配置 SP到EX、EX到租户 底价 含因子 个人或企业·POBO·阶梯
    TP->>EX: 配置 租户到客户 费率 同因子
    EX->>EX: 校验对外费率不低于上层底价
    Note over EX: 低于底价则拦截·不予保存
    CUST->>EX: 发起交易 换汇或调拨或结汇
    EX->>EX: 按因子匹配生效费率 收款人·POBO·阶梯·维度
    EX->>EX: 按对外费率计对客费额·按底价计成本

    alt 体系外SP 鲲鹏
        EX->>KUN: 同步对客费额 custFeeAmount
        KUN->>KUN: 按对客口径实扣·实际入账=EX对客展示
        KUN-->>EX: 回传实际扣费结果
        Note over EX,KUN: 差额 对客价减底价 按 UC-10 回结算给 EX或租户
    else 体系内SP A或B
        EX->>AB: 直接在共享账本落地
    end

    EX-->>CUST: 落账单与流水·返回交易结果与费用明细
```
...
1. 商户（已具有结汇能力，租户签约的s进入收款人管理，添加人民币（CNY）收款人（个人 / 企业）。
...
- [X] EX 切割与风险披露，避免 BB 风险牵连 EX 与租户品牌。—— 无风险。**VA 客户感知与自动申请 VA 的合规 / 法律风险**（关联 §二·补充「VA 收款」）：
  - **诉求**：客户最好仍需**感知 VA 的存在**（这是一个属于客户 / 收款人名下的账户），不宜完全黑盒。
  - **风险点①（法律 / 合规）**：由 EX / 系统"自动"替客户申请一个 VA，可能涉及**代客开户 / 账户实名与授权**问题——VA 归属主体、开户授权、KYC 责任需厘清，避免被认定为未经授权代客开立账户。
  - **风险点②（调单不知情）**：若结汇 SP 事后**调单 / 稽核**，客户不清楚资金为何进了某个 VA、为何被调单，缺乏可解释性与知情同意。
  - **待定方向**：VA 申请需**客户显式知情并授权**（首次弹出说明 + 授权签署），VA 号 / 归属对客户可见；明确 VA 开户主体与 KYC 责任方；调单场景给客户可见的说明与状态。
    ---------------------------------------------------------------------------------------------------------------------------------------------------------
