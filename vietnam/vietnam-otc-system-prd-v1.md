...
注：黑名单/灰名单命中仍参与综合风险定级，但本期不提供独立“名单命中”页签；“待复核交易”本期也不提供独立页签，相关结论在客户风险等级、KYT 查询记录和交易详情中展示。
...
| 资金管理 | §9      | 含渠道信息、收款工具、渠道头寸、头寸调拨、兑换/换汇 |
...
```text
任一候选账户可出账头寸 ≥ 客户应得金额 + 预计出账手续费
→ 头寸充足

所有候选账户均不足，但允许拆分出账且合计足够
→ MVP 进入人工确认，不自动拆分
...
MVP 默认一笔客户交易从一个渠道账户完成出账，不自动跨多个账户拆分。
...
| 人工确认                       | 无限额、超限或余额数据过期            |
| 客户限额允许，但可出账头寸不足 | 客户限额允许，但可出账头寸不足        |
...
本期不提供独立“名单命中”页签。名单检查仍在新增客户和交易提交时自动执行，命中摘要只在以下位置展示：

- 客户风险等级列表和客户详情风险提示卡片；
- 交易详情的风控快照；
- KYT 查询记录中的风险标签。

名单服务商原始证据链、分数和独立命中记录工作台均不在本期展示。
...
本期不提供独立“待复核交易”页签。需复核交易继续出现在客户交易列表中，状态为“风控复核中”，操作员从交易详情完成复核。
...
原始手续费 = 100,000,000 × 0.5% = 500,000 VND

实际手续费 = min(max(500,000, 50,000), 300,000) = 300,000 VND

客户实际用于兑汇的金额 = 99,700,000 VND
...
- 按比例或混合收费时，实际手续费按 `min(max(原始手续费, 最低手续费), 最高手续费)` 计算；最低或最高值留空时不应用对应边界；
- 原始手续费超过最高手续费时，按最高手续费收取，不阻断交易，不转人工审批；试算页需展示“已按最高收费封顶”；
- 固定金额收费不应用最低/最高手续费边界；混合收费先汇总比例部分和固定部分，再统一应用上下限；
- 手续费不得大于客户支付金额。若实际手续费大于等于客户支付金额，系统禁止提交并提示客户提高交易金额；
- 客户试算页面展示原始手续费、最低/最高边界、实际手续费和“预计获得金额（已扣手续费）”；
- 创建交易时保存收费类型、费率、固定金额、上下限、原始手续费、实际手续费及是否触发封顶的完整快照；
- 专属配置变更不影响已创建交易的报价快照。
...
OTC 可以定制关注的货币对列表，系统定期获取并展示各渠道对应汇率，帮助 OTC 快速发现出账机会。
...
OTC 除了给客户出币/出款外，还会使用自己的渠道账户进行兑换、换汇、买卖 U、充值、提现和账户间划转。这些属于 OTC 自有资金运营，不属于客户交易。
...
一笔渠道兑换可以用于补充多个客户交易的头寸；一笔客户交易也可以关联一笔调拨或渠道兑换，但两者不能共用同一个订单状态。
...
三层对账相互关联但独立显示。客户交易已完成，不代表渠道兑换或账户余额一定已经对平。
...
OTC 在渠道后台完成的任何兑换、调拨、付款或出币，必须通过以下任一方式进入 EX：
...
| 操作员 | 客户、交易、报价、入账、出账、调拨操作；不能管理内部用户和停止合作 |
...
| 客户费率配置      | 专属 Markup、手续费、上下限、封顶试算、启用/停用                                          |
| 风控管理          | 客户风险等级列表、调整记录、KYT 查询记录；名单命中和待复核不设独立页签                    |
| 参考汇率/对客报价 | 参考汇率来源、新建、编辑、复制、停用                                                      |
| 汇率关注          | 关注货币对、渠道汇率采集、刷新、直接去出账                                                |
...
4. OTC 可以查看客户风险等级列表、调整等级并记录原因；黑名单/灰名单命中参与定级，但本期不提供独立命中记录页签；
5. OTC 可以为交易钱包地址/账户发起 KYT 查询、查看“能做/需复核/不能做”结论，并查看 KYT 查询记录；
6. OTC 可以为客户设置单笔和日累计限额；
7. OTC 可以为客户配置专属 Markup 和手续费；比例或混合手续费支持最低/最高边界，超过最高收费时自动封顶且不阻断交易；
...
13. OTC 可以配置汇率关注列表，系统定期采集各渠道关注货币对汇率并支持“直接去出账”；
14. 完成交易后可以进行基础金额对账；
