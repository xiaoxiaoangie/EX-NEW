...
客户从 BB MP 完成 KYC/KYB 并经 SP 路由命中 B 后，由 B 进行合规审核。本期仅对 **B 合规审核通过且场景类型为“货代”或“广告”**的客户，将 B 审核结果、场景类型及 KYC/KYB 材料推送至 A 入网开户，并接收 A 的开户结果。
...
```mermaid
flowchart TD
    C["客户从 BB MP 入网"] --> SP{"SP 路由"}
    SP -->|"命中 B"| BKYC["B 合规审核"]
    SP -->|"未命中 B"| OUT["不在本方案范围"]
    BKYC -->|"审核通过"| BOK["B 合规审核通过"]
    BOK --> BR{"场景类型是否为<br/>货代或广告"}
    BKYC -->|"未通过"| BO["不提交 A"]
    BR -->|"是"| AON["向 A 推送审核结果、场景类型<br/>及 KYC/KYB 材料"]
    BR -->|"否"| BO2["不提交 A，按 B 其他规则处理"]
    AON --> AR{"A 回传入网结果"}
...
1. 客户从 BB MP 申请入网并提交 KYC/KYB，SP 路由结果须命中 B。
2. B 完成合规审核；仅当审核通过且场景类型为“货代”或“广告”时触发 A 入网推送。
3. B 向 A 提交自身审核结果、场景类型及客户 KYC/KYB 材料；A 可返回审核中、待补件、通过、拒绝等结果。
...
sequenceDiagram
    ...
    C->>MP: 提交 KYC/KYB
    MP->>B: SP 路由命中 B，提交客户入网申请
    B->>B: 执行 B 合规审核
    alt B 审核通过且场景类型为货代或广告
        B->>A: 推送审核结果 + 场景类型 + KYC/KYB 材料
        A-->>B: 回传审核中/待补件/通过/拒绝
        B-->>MP: 同步能力状态
    else B 审核未通过或场景类型不匹配
        B-->>MP: 不推送 A
    end
...
1. SP 路由命中 B 后，只有 B 合规审核通过且场景类型为“货代”或“广告”的客户才向 A 提交入网；提交内容包含 B 审核结果、场景类型及 KYC/KYB 材料。
