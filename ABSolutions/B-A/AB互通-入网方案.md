...
- B 侧新增渠道 A（法币充值 / 收款 / 付款）；**并非所有 SP=B 的客户都推送 A**。本期推送条件为：**SP 路由命中 B、B 合规审核通过，且场景类型为“货代”或“广告”**。满足条件后，B 将自身审核结果、场景类型及客户 KYC/KYB 材料推送至 A。
...
flowchart TD
    ...
    B1 --> B2{"B 合规是否审核通过"}
    B2 -->|"否"| BFAIL["不推送 A"]
    B2 -->|"是"| SCENE{"场景类型是否为<br/>货代或广告"}
    SCENE -->|"是"| PUSH["推送 B 审核结果 + 场景类型<br/>+ KYC/KYB 材料至 A"]
    SCENE -->|"否"| OTH["不推送 A，按 B 其他规则处理"]
...
2. **判断是否推送 A**：仅当 SP 路由命中 B、B 合规审核通过，且场景类型属于“货代”或“广告”时才推送 A，**不做全量推送**；
3. 推送内容至少包括：**B 合规审核结果、场景类型、客户主体信息及 KYC/KYB 材料**（如 BR/CR 等）；审核结果需包含结论、审核时间及可追溯标识，场景类型使用约定枚举值；
...
- SP 路由未命中 B、B 合规审核未通过或场景类型不是“货代/广告”时，均不得推送 A；
- 满足推送条件时，B 向 A 传递的审核结果、场景类型及 KYC/KYB 材料完整，且可按审核标识追溯；
...
| 2    | 场景类型字段来源、枚举编码及“货代/广告”与 A 侧枚举的映射关系      | 待定义     |
