...
flowchart TD
    ...
    BSP --> BCOM{"B 合规审核是否通过"}
    BCOM -->|"否"| BREJ["不推送 A"]
    BCOM -->|"是"| CAP{"场景类型是否为<br/>货代或广告"}
    CAP -->|"是"| PUSHA["推送 B 审核结果 + 场景类型<br/>+ KYC/KYB 材料至 A"]
    CAP -->|"否"| OTHERCH["不推送 A，按 B 其他规则处理"]
...
| 2 | **入网改造** | 佳琛 | 客户入网链路需支持「SP 路由命中 B → B 合规审核通过 → 场景类型为货代/广告 → 推送 B 审核结果、场景类型及 KYC/KYB 材料至 A」的分支逻辑 |
