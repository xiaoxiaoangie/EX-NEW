...
| 范围        | 说明                                                 |
| ----------- | ---------------------------------------------------- |
| ✅ 本期包含 | OffRamp（卖出数币：数币→法币）A1 + B1 场景       |
| ❌ 本期不含 | OnRamp（买入数币：法币→数币）— 见 `onramp-v1.md` |
| ❌ 本期不含 | 纯 FX 兑换（法币↔法币）                             |
| ❌ 本期不含 | 纯提币（数币→外部钱包，无承兑）                     |
| ❌ 本期不含 | 纯提现（法币→外部银行，无承兑）                     |
| ❌ 本期不含 | A2 场景：承兑+跨 SP 划转到 IPL（Phase 2）          |
| ❌ 本期不含 | B2 场景：承兑+跨 SP 划转+IPL 付款（Phase 2）        |
...
> ⚠️ **本期范围（Phase 1）：仅 A1 + B1**。A2、B2（涉及跨 SP 划转）均放入 Phase 2。
> **设计原则**：承兑业务的法币必须留在承兑交易账户（BB）内，不允许在 OffRamp 流程中跨 SP 划转到收付款账户（IPL）。
> 如商户需要将承兑后的法币从承兑交易账户转到收付款账户，应通过**独立的账户间划转**操作完成，不属于 OffRamp 范畴。
...
- **卖出数币带付款场景**：需要调用外部渠道付款给收款人（本期仅 BB/XPAY 渠道；IPL 法币通道 = Phase 2）
...
| **IPL 收付款法币账户** | IPL | 非承兑业务的法币收付款（Payins/Payouts 等） | OffRamp 本期不涉及（A2/B2 Phase 2） |

### 3.3 卖出数币（OffRamp）资金流规则

```
规则1（单 SP 承兑）：所有卖出数币承兑必须从 BB 数币钱包 → BB 承兑法币账户
规则2（账户封闭）：承兑后法币留在 BB 承兑交易账户，OffRamp 流程中不跨 SP 划转
规则3（付款渠道）：本期仅走 BB/XPAY 渠道（IPL 渠道付款 = Phase 2）
规则4（跨账户划转）：BB 承兑法币账户 → IPL 收付款账户 是独立操作，不属于 OffRamp 范畴
```
...
| A2   | BB+IPL | 纯承兑+跨SP划转    | 1      | 2 (BB 承兑 + BB→IPL 跨SP划转)            | 0          | 商户即时发起 | ⚠️ **Phase 2** |
| B1   | 仅 BB  | 承兑+提现          | 1      | 2 (BB 承兑 + BB 付款)                     | 1 (BB PP)  | 商户即时发起 |
| B2   | BB+IPL | 承兑+跨SP划转+提现 | 1      | 3 (BB 承兑 + BB→IPL 跨SP划转 + IPL 付款) | 1 (IPL PP) | 商户即时发起 | ⚠️ **Phase 2** |
...
卖出数币（OffRamp）1.0 场景矩阵（单SP承兑策略）：
                    ...
                    仅BB（单SP）              BB+IPL（双SP）⚠️ Phase 2
                ┌──────────────────┐     ┌────────────────────────────────┐
纯承兑          │ A1: USDT→BB USD  │     │ A2: USDT→BB USD→IPL USD      │
（到法币账户）  │ 1笔交易单 ✅     │     │ 2笔交易单 ⚠️ Phase 2 不做    │
                └──────────────────┘     └────────────────────────────────┘
                ┌──────────────────┐     ┌────────────────────────────────┐
带付款          │ B1: USDT→USD→付款│     │ B2: USDT→BB→IPL USD→付款      │
（给收款人）    │ 2笔交易单 ✅     │     │ 3笔交易单 ⚠️ Phase 2 不做    │
                └──────────────────┘     └────────────────────────────────┘

⚠️ 本期范围（Phase 1）= A1 + B1（纯 BB 内部）
⚠️ A2/B2 均放入 Phase 2：承兑后法币必须留在承兑交易账户，不在 OffRamp 流程中跨 SP
```
...
### 5.3 场景 A2：即时单 — BB 承兑法币账户→IPL 收付款账户（BB+IPL）【Phase 2 — 本期不做】

> 🚧 **Phase 2 场景**：本场景涉及承兑后跨 SP 划转，违反“承兑法币留在承兑交易账户”的设计原则，放入下一期。如商户需要将承兑后法币转到收付款账户，应通过独立的账户间划转操作完成。
>
