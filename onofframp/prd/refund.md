...
- **退回不再收承兑手续费**：承兑后付款失败退回数币钱包时，不再收承兑手续费；但默认付款手续费不退

**商户退回配置（承兑相关）：**

商户需配置退回时的汇率规则，决定承兑后付款失败退回时退回数币还是法币：

```
退回汇率规则算法（Off-Ramp退回：原交易 USDT→USD，付款失败需退回）：

输入：
  original_rate = 原交易时 1 USDT 的 USD 价格
  realtime_rate = 退回时 1 USDT 的 USD 价格
...
> 这是最复杂的退回场景。商户先做了承兑（数币→法币），然后付款失败，需要退回。
> 核心问题：退回数币还是法币？用什么汇率？
>
...
```
Off-Ramp退回（原交易 USDT→USD，付款失败需退回）：
  original_rate = 原交易时 1 USDT = X USD
  realtime_rate = 退回时 1 USDT = Y USD
...
**场景：** 商户从BB数币钱包直接Off-Ramp通过IPL付款（offramp-v1.md 场景A2），IPL付款失败，需退回到BB数币钱包。
...
| 1.3 | 充币-成功后退款-原地址       | 商户发起(独立交易) | 原地址(KYT复查) | 1笔RT+1笔RC | 独立退款费率 | RM001+原单REFUNDED |
| 1.3 | 充币-成功后退款-新地址       | 商户发起(独立交易) | 新地址(KYT+7天) | 1笔RT+1笔RC | 退款费+Gas | RM001+原单REFUNDED |
| 2.1 | VA收款-渠道拒绝              | 合规操作       | 原汇款人         | 无(渠道层)  | 不收       | 无单据    |
| 2.2 | VA收款-风控拦截              | 销售+清算+合规 | 原路/新人        | 1笔RT       | 清算确认   | 1笔退款单 |
| 2.3 | VA收款-成功后退款-原路     | 商户发起(独立交易) | 原汇款人(制裁复查) | 1笔RT+1笔RC | 独立退款费率 | RM001+原单REFUNDED |
| 2.3 | VA收款-成功后退款-新收款人 | 商户发起(独立交易) | 新收款人(KYC/KYB) | 1笔RT+1笔RC | 退款费+汇款费 | RM001+原单REFUNDED |
...
*最后更新：2026-02-25*
*文档版本：v3.2 — 重写1.3/2.3为独立交易类型(CRYPTO_DEPOSIT_REFUND / VA_COLLECTION_REFUND)，走完整业务链路：业务校验→交易引擎→计费→记账(冻结)→风控(KYT)→渠道单→记账(确认)→通知*
