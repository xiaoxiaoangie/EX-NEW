...
```
本期不存在系统自动路由，商户在下单时自行选择：

1. OnRamp 即时单（场景A）：商户自己选支付来源
   - 只有 BB 法币账户 → 只能选 BB 余额（场景A1）
   - 只有 IPL 法币账户 → 只能选 IPL 余额（场景A2）
   - BB+IPL 都有 → 商户自己选从哪个法币账户扣款

2. OnRamp 预约单（场景B）：商户自己选收款账户
   - 只有 BB → 展示 BB 代收付账户（场景B1）
   - 只有 IPL → 展示 IPL VA 账户（场景B2）
   - BB+IPL 都有 → 商户自己选用哪个收款账户

3. OffRamp 付款：资金在哪个 SP 账户就从哪出
   - 不存在跨 SP 路由选择

本期不实现：系统自动路由、TP 配置优先级、商户级路由配置
```

> 详细的开通规则、租户签约配置、入网推送逻辑见 `productionopening.md` 第1~3章。
...
sequenceDiagram
    ...
    rect rgb(255, 250, 240)
        Note over HUB,Account: 阶段3：创建交易单+冻结余额
        BL->>TE: 6. 请求交易引擎
        TE->>TE: 7. 创建交易单T001(BB-承兑)
        TE->>Account: 8. 冻结法币余额(商户BB USD账户)
        Account-->>TE: 9. 冻结成功
    end
    ...
    rect rgb(240, 255, 240)
        ...
        par 并行执行
            TE->>Pricing: 10a. 计算承兑费用
            Pricing-->>TE: 11a. 返回费用明细
        and
            TE->>FX: 10b. 查询实时汇率(USD/USDT)
            FX-->>TE: 11b. 返回商户汇率
        end
        TE->>TE: 12. 更新交易单(费用/汇率)
    end
  
    rect rgb(230, 240, 255)
        Note over HUB,Account: 阶段5：BB承兑风控
        TE->>Risk: 13. BB承兑风控检查(商户风险等级、交易频率、金额等)
        Risk-->>TE: 14. 风控通过
    end
  
    rect rgb(255, 245, 238)
        Note over HUB,Account: 阶段6：记账（内部划转）
        TE->>Account: 15. 确认扣款(冻结→扣除, 商户BB USD账户-)
        TE->>Account: 16. 入账(商户BB USDT钱包+)
        Account-->>TE: 17. 划转成功
...
| BB承兑风控拒绝 | BB风控检测到异常（商户风险等级、交易频率等） | 解冻已冻结的法币余额，商户单=FAILED                    | 提示"风控审核未通过"       |
...
| -------------------------------------- | -------------------------------------- | ---------------------------------------------------------------- | --------------------------- | --------------------- |
| 业务校验/冻结失败                      | 余额不足/产品未启用/超限额/并发冻结    | 直接拒绝或解冻，不产生退款                                       | 无                          | 下单失败              |
| IPL出款风控拒绝                        | IPL风控检测到出款异常（合规/风险等级） | 解冻商户IPL法币余额，T001=FAILED，T002不创建                     | 无（解冻即恢复）            | 提示"风控审核未通过"  |
...
| 收款风控拦截（T001阶段）   | BB收款风控拦截（汇款人AML不通过/可疑交易） | 资金入账但冻结→通知合规→销售联系商户→清算确认手续费→退回原汇款人 | 退回汇款人（参考refund.md 2.2） | "收款被风控拦截，正在处理退回" |
...
| IPL收款风控拦截（T001阶段）               | IPL收款风控拦截（汇款人AML不通过等）    | 资金入账但冻结→合规→销售→清算→退回原汇款人                      | 退回汇款人（参考refund.md 2.2） | "收款被风控拦截"                     |
| 预约单匹配失败                            | 非同名/缺Purpose Code/无预约单/金额偏差 | 详见第10章"预约单匹配规则"异常处理                                  | 视情况                          | 视情况                               |
...
| - | ---- | ----------------------- | -------------- | ----------------- | ---------- | ---------- | ---------------- |
| 1 | A1   | 业务校验/冻结/风控/承兑 | 未扣款或已冻结 | 解冻退回BB法币    | ❌         | 不收       | 订单失败         |
| 2 | A2   | 业务校验/冻结/IPL风控   | 未扣款或已冻结 | 解冻退回IPL法币   | ❌         | 不收       | 订单失败         |
| 3 | A2   | BB承兑风控/执行失败     | 中间户         | 逆向回滚→IPL法币 | ✅(RT内部) | 不收       | 订单退款         |
| 4 | B1   | 收款风控拦截            | 冻结在BB       | 退回原汇款人      | ✅         | 清算确认   | 退款通知         |
| 5 | B1   | BB承兑风控/执行失败     | 商户BB法币     | 留在BB法币账户    | ❌         | 不收       | 订单失败，可重试 |
| 6 | B2   | IPL收款风控拦截         | 冻结在IPL      | 退回原汇款人      | ✅         | 清算确认   | 退款通知         |
| 7 | B2   | IPL出款风控/提现失败    | 商户IPL法币    | 留在IPL法币账户   | ❌         | 不收       | 订单失败，可重试 |
| 8 | B2   | BB承兑风控/执行失败     | 中间户         | 逆向回滚→IPL法币 | ✅(RT内部) | 不收承兑费 | 订单退款         |

**核心原则总结：**

1. **资金未离开商户账户** → 解冻/回滚即可，不产生退款单
2. **资金在中间户**（跨SP场景） → 逆向回滚，创建内部退款交易单，不收承兑手续费
3. **收款风控拦截** → 退回原汇款人，参考 `refund.md` 2.2 流程，清算确认手续费
4. **预约单收款成功但承兑失败** → 资金留在商户法币账户，商户可用余额重新发起OnRamp
