...
| 行业/业务品类 | 电商、广告、物流等 | 入网、定期复审 |
| B 入网状态 | 正常、暂停、关闭 | B 客户系统实时/准实时同步 |
| A 直接服务结果 | 可服务、不可服务、待补充 | A 能力规则或准入结果 |
| MOR 关系状态 | 未建立、审核中、有效、冻结、过期 | MOR 运营系统 |
| 渠道关系状态 | 正常、限额、冻结、关户 | 渠道回调与运营维护 |
...
- A 不直接服务、MOR 路径已就绪：展示“专属出款 + 1:1 平价兑换”。
- A 不直接服务且 MOR 路径未就绪：仅展示“1:1 平价兑换”；若 1:1 渠道也不可用则不提供可成交报价。

第二层由客户在系统筛选后的选项中选择。该设计避免标准出款与专属出款同时出现时，客户必然选择更便宜的标准出款；也避免 A 可直接服务的客户因“丝滑”宣传被不必要地导向 MOR。
...
- A 不直接承接但 MOR 路径已就绪的客户看到专属出款和固定兑换版。
...
“预约付款”比“余额付款”更准确：客户并非使用已有余额，而是先明确付款目标、收款人和兑换方式，再向本次业务分配的地址充值。
...
3. **全成本报价**：路由成本模型统一纳入钱包费、链上费、调拨费、承兑价差、付款费、MOR 成本、失败/退票预期成本和资金占用成本。
...
flowchart TD
    ...
    C -->|A不直服且MOR就绪| E["展示专属出款 + 1:1"]
    C -->|A/MOR不可用| E1["仅展示1:1或无报价"]
    D --> F0["选择钱包付款或预约付款"]
    E --> F0
    E1 --> F0
...
每个客户维护 B 入网状态、动态标签、A 直接服务结果、MOR 关系、渠道关系、可用主路径、1:1 权限、POBO/非 POBO 权限、额度和有效期。
...
路由输入包括客户动态标签、A 服务结果、MOR 关系、付款发起方式、兑换方式、付款模式、币种国家、金额、渠道客户状态、是否要求配套钱包、渠道余额/额度、调拨成本、可用性、SLA、黑名单和规则版本。
...
1. 系统依据客户国家、行业/品类、A 服务结果、MOR 关系、渠道关户及交易实时条件生成动态标签，并可追溯标签来源、版本和有效期。
2. 渠道能力中心可维护国家、品类、币种、金额、付款模式、钱包要求、费率、调拨成本、限额、可用性和客户渠道状态。
3. A 可直接服务的客户只看到“标准出款 + 1:1”；A 不直接服务且 MOR 路径就绪的客户只看到“专属出款 + 1:1”，同一客户不得同时看到标准出款和专属出款。
...
10. MOR 客户可在无 POBO 备案的情况下，通过独立且有效的 MOR 关系使用已批准的非 POBO 能力。
11. MOR 关系失效时，即使 POBO 付款人仍有效，专属出款路径也必须拦截；反之 POBO 失效不得无理由关闭非 POBO 权限。
12. 重复请求、重复回调和结果未知不会造成重复 Convert 或重复法币付款。
13. A 不直接承接的专属出款订单不可静默回落至 A 的标准直连路径；预约地址分配后不得无感切换钱包渠道。
14. 渠道出款失败或退票后，可追踪原订单、退回资金、费用、处理责任方及最终结果。
15. 未完成协议、合规及资金安全批准的 1:1 渠道不能被配置为生产可用。
