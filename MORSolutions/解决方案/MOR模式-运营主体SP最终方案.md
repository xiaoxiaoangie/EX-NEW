...
>
> **绑定成功条件**：客户与 MOR 主体完成分配和材料上传后，还必须在 A 添加 POBO 付款人成功，客户—MOR 主体关系才可标记为“绑定成功”。
...
7. MOR 运营系统使用该 MOR 主体在 A 的 MID，向 A 添加客户为 POBO 付款人；
8. A 返回添加成功后，MOR 运营系统才将关系标记为“绑定成功”；
9. MOR 运营系统向 B 返回 MOR 主体名称、企业证件编号及 MOR 主体在 B 的 MID；
10. 客户在 B 发起 OffRamp，数币通过 B 钱包 A2A 转入该 MOR 主体在 B 的钱包；
11. B 使用该 MOR 主体在 A 的 MID及已审核通过的 POBO 付款人信息创建 A 出款；
12. A 向客户本人或经审核的收款账户支付法币。
...
4. 将 A POBO 付款人添加成功作为关系绑定成功的必要条件；
5. 确保数币进入当前绑定 MOR 主体在 B 的钱包；
6. 确保 A 出款使用同一个 MOR 主体在 A 的 MID；
7. 保留客户、关系、材料、B MID、A MID、POBO 付款人和交易快照。
...
6. 使用该 MOR 主体 A MID向 A 添加客户为 POBO 付款人成功；
7. A 已返回有效的 POBO 付款人 ID或等效成功标识。

前五项完成但 A POBO 尚未成功时，只能处于“待 A POBO 审核”，不能向 B 返回绑定成功结果，也不能用于 OffRamp。
...
sequenceDiagram
    ...
    alt A添加成功
        ...
        A-->>OPS: 返回RFI
        OPS->>OPS: 关系保持待A POBO审核或待补件
    else A添加失败
        A-->>OPS: 返回失败原因
        OPS->>OPS: 关系标记为POBO失败，不得绑定成功
    end
```
...
- “待分配”“待材料”“待 A POBO 审核”“待补件”和“POBO 失败”均不得用于 OffRamp；
- 只有“绑定成功”关系才计入 MOR 主体 10 个有效客户名额；
- A POBO 添加失败后可在材料修正并通过审核后重新提交；
- 关系失效不修改历史交易快照。
...
- A POBO 添加失败时，关系不得标记为绑定成功；
- 已绑定客户换绑时，新 MOR 主体必须重新完成材料审核和 A POBO 添加；
- 新关系达到绑定成功后，旧关系才可失效；
- 在途交易继续使用交易创建时保存的原 MOR 主体、B MID、A MID及 POBO 付款人快照。
...
sequenceDiagram
    ...
    C->>B: 发起OffRamp
    B->>B: 查询绑定成功的MOR关系
    B->>OPS: 必要时校验关系和POBO状态
    OPS-->>B: 返回绑定成功状态及MOR主体信息
    B->>B: 根据B MID定位MOR主体钱包
    B-->>C: 返回数币支付信息
    C->>B: 支付数币
    B->>MW: 钱包A2A转入对应MOR主体
    B->>A: 使用同一MOR主体A MID及POBO付款人创建出款
    A->>P: 支付法币
    A-->>B: 返回出款结果
    B-->>C: 展示交易结果
```
...
OffRamp：B → 使用同一 MOR 主体的 A MID和已审核 POBO 付款人 → A
法币：A → 客户本人或经审核的法币收款账户
```
...
| 接口 | 用途 |
| --- | --- |
| 创建付款 | 使用具体 MOR 主体 A MID及已审核的 POBO 付款人出款 |
| 付款状态查询 | 处理回调丢失或结果未知 |
| 退款状态查询 | 查询失败后的资金退回结果 |
...
5. **绑定成功门槛**：A POBO 付款人未添加成功，关系不得标记为绑定成功。
...
6. A POBO 付款人添加成功前，关系不得标记为绑定成功；
7. A 返回 POBO 添加成功后，MOR 运营系统才将关系改为绑定成功；
...
12. A 出款使用同一 MOR 主体的 A MID及该客户已审核通过的 POBO 付款人；
13. 系统必须阻止使用 MOR 运营方或其他 MOR 主体的 MID；
14. A 结果未知时不得重复付款；
15. 本期流程中不得出现 Cregis 路由或渠道切换；
16. 历史交易保留 B MID、A MID、POBO 付款人和材料快照；
17. 全链路可按客户、MOR 主体和交易完成查询、审计及对账；
18. 每个绑定关系能够说明 MOR 的具体业务场景，并关联对客交易责任及下游采购、授权、分销或服务关系证据；仅有资金关系时不得完成绑定。
