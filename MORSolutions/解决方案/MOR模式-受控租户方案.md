...
> 完整链路：客户入网提交 KYC+KYB → 系统按 industries 映射分流 → 不支持的 industries 推送对应租户 → 租户关联 MOR 并上传 KYB 材料 → 报备 A。报备结果统一三态：**通过 / 需补充 / 拒绝**；**拒绝时系统不展示具体原因，由 MOR 运营（租户侧）向客户解释**。**全程客户不感知 MOR。**
...
sequenceDiagram
    ...
    A-->>TEN: 报备结果（通过/需补充/拒绝）
    alt 通过
        TEN-->>C: 入网成功（客户不感知MOR）
    else 需补充
        TEN->>TEN: 用MOR材料补充/通知客户补充
        TEN->>A: 再次提交
    else 拒绝
        TEN-->>C: 统一拒绝话术（MOR关联客户不暴露A原文）
    end
```
...
- **A 报备结果统一三态**：**通过 / 需补充 / 拒绝**；
  - **通过**：租户通知客户入网成功；
  - **需补充**：走 RFI 内循环，租户用 MOR 材料补充或通知客户补充，**不落对客「驳回」状态**；
  - **拒绝**：A 给出拒绝原因，但 **系统不展示给客户**；由 **MOR 运营（租户侧）根据统一口径向客户解释**，必要时线下沟通；对直客（未关联 MOR）可按既有口径展示；
- **客户全程不感知 MOR**：客户只见自己上传的材料，MOR 关联客户不感知 MOR 材料及 A 原文拒绝原因；
...
- 四个视角的可见范围与上表一致；
- A 报备结果三态（通过/需补充/拒绝）处理正确：
  - 通过：租户通知客户入网成功；
  - 需补充：走 RFI 内循环，租户再次提交；
  - 拒绝：系统不展示具体原因，由 MOR 运营（租户侧）向客户解释；
...
> A 合规对交易/客户发起调单，B 运营统一转租户，由租户内部判断是否关联 MOR 并提交材料。沿用统一三态口径：**通过 / 需补充（RFI 内循环） / 拒绝**；**拒绝时系统不展示具体原因，由 MOR 运营（租户侧）向客户解释**。
...
sequenceDiagram
    ...
    BC->>BO: 拒绝原因
    alt 已关联MOR
        BO->>TEN: 不展示具体原因 转 MOR 运营解释
        TEN-->>C: MOR 运营按统一口径向客户解释（不暴露A原文）
    else 未关联MOR（直客）
        BO->>TEN: 可按既有口径转述原因
        TEN-->>C: 通知客户
    end
...
- **A 复核结果统一三态**：**通过 / 需补充（退回 RFI 内循环）/ 拒绝**；
  - **通过**：交易/客户继续正常处理；
  - **需补充**：走 RFI 内循环，由租户再次提交（已关联 MOR 用 MOR 材料、未关联通知客户），**不落对客「驳回」状态**；
  - **拒绝**：A 给出拒绝原因，但 **系统不展示给客户**；由 **MOR 运营（租户侧）按统一口径向客户解释**，必要时线下沟通；对直客（未关联 MOR）可按既有口径展示；
...
- A 复核三态（通过/需补充/拒绝）处理正确：
  - 通过：继续后续流程；
  - 需补充：走 RFI 内循环，由租户再次提交；
  - 拒绝：系统不展示具体原因，由 MOR 运营（租户侧）向客户解释；
- 可见性与关系变更时间点规则正确。
