...
> 文档状态：待产品、风控、合规、法务及技术评审  
> 需求主体：A  
> 适用范围：A 的 POBO 付款人管理、A 风控审核及 A 付款能力  
> 系统边界：本能力仅建设在 A，不涉及 EX 的页面、风控、数据存储、付款编排或接口  
> 核心原则：所有 POBO 付款人必须先经过 A 风控审核；审核通过不等于任意交易均可使用，付款时仍需进行交易级风控。
...
| 术语 | 定义 |
| --- | --- |
| A | POBO 付款人建档、审核、付款和渠道对接的唯一能力主体 |
...
- 在 A 内统一付款人准入和风险控制，避免通过线下方式传递付款人名称；
...
3. 所有付款人先向 A 提交资料，再由 A 风控审核；
4. 仅审核成功、状态正常且在有效期内的付款人可被付款订单引用；
5. 付款时再次校验业务关系、收款对象、国家、币种、金额、频率和通道能力；
...
|  |
| A 风控审核员 | 查看完整资料、筛查结果，执行通过、补件、拒绝、冻结 |
| A 风控复核员 | 对高风险或超阈值申请进行二次复核，不得与初审人为同一人 |
| A 运营 |  联系客户/销售按照合规要求补充/修改相应信息
|
...
flowchart TD
    ...
    OK --> PAY["付款时选择POBO付款人"]
    PAY --> TR["交易级风控"]
    TR -->|"通过"| OUT["提交A及实际付款通道"]
    TR -->|"拦截"| HOLD["禁止付款或转人工审核"]
```
...
| 最近使用时间 | 最近被付款订单引用的时间 |
| 创建时间 | 首次创建时间 |
| 操作 | 详情、补件、编辑、停用、查看使用记录 |

规则：

- 审核中不可直接编辑；如需修改，撤回后生成新版本；
- 已通过付款人的关键字段发生变化后，原版本立即停止新增付款引用，新版本重新审核；
- 已被历史订单引用的付款人不可物理删除；
...
| 预计国家/币种 | 是 | 多选；作为风控和通道能力校验条件 |
| 预计单笔/每月金额 | 是 | 作为额度和增强尽调依据 |
...
个人 POBO 默认进入增强审核，不允许仅凭姓名和证件号码启用。
...
商户侧不展示制裁、可疑交易监控模型、内部名单命中细节或风控敏感原因；仅展示标准化结果及可补充项。
...
- 身份/KYB/KYC 信息真实性和完整性；
- 企业注册状态、董事、法人、受益人和实际控制人识别；
- 制裁、PEP、负面新闻、内部黑名单及高风险名单筛查；
- 国家/地区、行业、经营范围和付款场景准入；
- 申请主体、付款人、收款人之间的关系合理性；
- 合同、订单、Invoice、授权书之间的名称、金额、币种和日期一致性；
- 预计交易规模与付款人业务规模是否匹配；
- 是否存在空壳主体、代持、循环交易、虚构贸易或资金过桥风险；
- 是否存在同证件、同名称、同受益人或同联系方式的重复/关联申请；
- 付款通道是否支持该类型、国家和信息字段。
...
2. 申请主体有权使用该付款人；
3. 当前收款人与已审核业务关系一致；
4. 付款国家、币种、金额和用途在批准范围内；
5. 订单、合同、Invoice 与付款指令匹配；
6. 未超过单笔、单日、单月及通道额度；
7. 付款人、收款人和相关方重新进行名单筛查；
8. 付款频率、拆单、集中度、异常时间和历史行为未命中交易监控规则；
9. 实际通道支持 POBO，且支持传递该付款人类型和所需字段；
10. 付款报文中的付款人名称必须来自已审核快照，不允许前端临时修改。
...
- 通道 RFI：关联原付款人审核材料和本笔交易材料，由 A 风控统一处理；
- 收款行退汇：记录是否因付款人不匹配、材料不足、收款限制或其他原因，并回流风险策略；
...
- 敏感信息按字段级加密和角色脱敏；
- 证件、受益人、名单筛查及审核数据仅授权角色可见；
- 审核和付款日志不可被普通运营修改；
- 历史订单引用快照不可因付款人后续编辑而变化；
- 列表和详情需要支持审计导出，导出需权限控制和水印；
- 国家、证件类型、行业、场景、关系和风险原因码均使用版本化字典；
...
3. 未通过 A 风控的付款人不能用于任何付款；
4. 审核通过但已冻结、过期或停用的付款人不能用于新付款；
5. 付款人关键资料修改后必须重新审核；
6. 付款时能按商户权限、国家、币种、场景和通道能力筛选可用付款人；
7. 每笔付款执行交易级风控并保存付款人版本快照；
8. A 能使用已审核的 POBO 付款人创建付款指令，并按实际通道映射付款人字段；
9. 通道不支持 POBO 时不得静默使用平台默认付款人名称；
10. 风控支持通过、待补件、拒绝、冻结及重新审核；
11. 名单筛查、受益人识别、合作材料和审核意见全链路可追溯；
12. 历史付款不受付款人后续修改、冻结或失效影响；
13. RFI、退汇和付款人不匹配原因可以回流至付款人及风险策略；
14. 商户侧不可查看风控敏感规则、名单命中详情和内部调查信息。
