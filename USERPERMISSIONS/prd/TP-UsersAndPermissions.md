...
> 版本：v1.4 | 更新日期：2026-03-21 | 关联页面：permissions-tp.html / tp-login.html / tp-personal-profile.html
...
TP 端**本期不支持 2FA**，安全验证规则为 MP 的简化版本。
...
| --- | --- |
| **修改密码** | 输入当前密码 → 输入新密码 |
| **修改手机号（已绑定）** | 向原手机号发送验证码 → 输入新手机号 → 验证新手机号 |
| **首次绑定手机号** | 输入新手机号 → 验证新手机号（无原手机可验，已登录态即身份证明） |

> **与 MP 的关系：** MP 在以上规则前增加「有2FA → 验2FA」的优先判断。TP 本期无 2FA，等同于 MP 的「没2FA」分支。后续 TP 如开启 2FA，直接复用 MP 规则。

### 5.3 手机号绑定/修改

```mermaid
flowchart TD
    A[用户点击 Phone 的 Bind / Edit 按钮] --> B{是否已绑定手机号}

    B -->|未绑定-首次绑定| C[输入新手机号<br/>选择区号 + 输入号码]
    B -->|已绑定-修改| D[Step 1: 向原手机号发送验证码]
...
- 修改手机号需先验证原手机号，防止未授权修改
- 本期不用于 2FA 或安全验证，后续版本可扩展为 2FA 渠道
...
> **与 MP 一致：** MP 端在没有 2FA 的情况下，密码修改流程也是「输入当前密码 → 输入新密码」，逻辑完全一致。
...
| T09  | 手机号变更验证码           | 短信      | 修改已绑定手机号时，向原手机号发送验证码 |
| T10  | 手机号变更成功通知         | 邮件      | 手机号绑定/修改成功后确认通知 |
...
> 修改已绑定手机号时，向**原手机号**发送验证码以验证身份。

| 语言     | 短信内容                                                                                                                  |
| -------- | ------------------------------------------------------------------------------------------------------------------------- |
| EN       | 【EX Platform】Your verification code is {code}. Valid for 5 minutes. Use this to verify your phone number change request. |
| 简体中文 | 【EX Platform】您的验证码是 {code}，5分钟内有效。用于验证手机号变更请求。                                                 |
| 繁體中文 | 【EX Platform】您的驗證碼係 {code}，5分鐘內有效。用於驗證手機號變更請求。                                                 |

#### T10 手机号变更成功通知

| 语言     | 邮件标题                         | 邮件正文摘要                                                                                                                                     |
| -------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| EN       | Your phone number has been changed | Your Tenant Portal phone number was successfully changed on {datetime}. If this wasn't you, please contact support immediately.                |
| 简体中文 | 您的手机号已变更                 | 您的租户管理后台绑定手机号已于 {datetime} 成功变更。如非本人操作，请立即联系客服。                                                                |
| 繁體中文 | 您的手機號已變更                 | 您的租戶管理後台綁定手機號已於 {datetime} 成功變更。如非本人操作，請即刻聯絡客服。                                                                |
...
| v1.4 | 2026-03-21 | 个人中心增加安全验证统一规则（5.2）；手机号绑定/修改拆分为首次绑定和修改两个分支，修改需先验原手机号（5.3）；密码修改补充与 MP 一致性说明（5.4）；通知新增 T09 手机号变更验证码、T10 手机号变更成功通知 |
