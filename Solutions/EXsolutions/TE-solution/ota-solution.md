...
OTA 平台希望**自建一套面向 C 端旅客的多币种钱包**，以替代纯卡支付，沉淀用户资金、提升复购：
...
| **支付旅行订单**   | C 端用钱包余额支付 OTA 平台的机票 / 酒店等订单（内部账本扣减）        |
...
| 角色                              | 说明                                                                                                             |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| **OTA 平台**             | 钱包发行方 + OTA 平台，在 EX 系统上以**商户**身份入网。**OTA 平台自己负责 C 端钱包账本**，EX 不感知 C 端 |
| **C 端旅客**                | 钱包终端用户。**EX 不感知 C 端**，C 端账户与多币种余额由 OTA 自行记账                                      |
| **SP**                      | **实际的 Crypto Checkout / 承兑 / 法币换汇 / 付款持牌机构**。OTA 的法币 / 数币账户真实开在 SP 处                   |
| **EX**                      | 技术 + 合规运营平台，提供 API、Webhook、SP 路由、风控、对账与合规管控                                            |
| **第三方收单机构（非 EX）** | Stripe / Adyen / dLocal / 本地 PSP 等。**EX 不提供 C 端法币卡 / 本地钱包收单**，需 OTA 自行对接            |
...
| 能力                              | 支持范围                                  | 用途                                 |
| --------------------------------- | ----------------------------------------- | ------------------------------------ |
| **Crypto Checkout（收 U）** | USDT / USDC（链上收款）                   | C 端数币充值钱包                     |
| **承兑 OffRamp（U → 法）** | 仅**USD**                           | 数币充值后数转法上账 USD 钱包        |
| **承兑 OnRamp（法 → U）**  | 仅**USD**                           | OTA 主体调拨：USD → USDT            |
| **法币 FX 换汇**            | USD ↔ 主要法币（具体币种以 SP 通道为准） | 钱包内 C 端跨币种兑换的底层结算      |
| **法币提现**                | 仅**USD**                           | OTA 主体 USD 提现到银行              |
| **数币提币**                | USDT / USDC 链上                          | OTA 主体数币出金                     |
| **VCC 发卡（可选）**        | 美卡 BIN                                  | 用于向 GDS / 航司 / 酒店等供应商结算 |

### 3.2 EX **不**提供的能力（OTA 平台自找）
...
> **法币充值通道由 OTA 平台自行接入**。EX 仅在数币侧（Crypto Checkout + 数转法 OffRamp）和 OTA 主体的法币侧（FX / Payout）提供能力。

---

## 四、资金托管模型

OTA 的法币 / 数币账户**真实开立在 SP 处**，EX 仅承担技术链路 + 合规编排：

```
OTA 数币账户（USDT/USDC）  ←─ 真实开在 SP，EX 通过 API 查询 / 提币
OTA 法币账户（USD）        ←─ 真实开在 SP，EX 通过 API 查询 / FX / 提现
```

**C 端钱包余额是 OTA 平台自家内部账本**，资金沉淀在 OTA 平台在 SP 的对应币种账户。无 "归集"，所有 C 端动作的资金影响最终落到 OTA 平台的 SP 账户。
...
flowchart LR
    ...
    classDef ota fill:#ecfdf5,stroke:#10b981,color:#1a1a2e
    ...
    subgraph OTA[OTA 钱包系统]
        WA[C 端多币种钱包账本<br/>USD EUR USDT ...]:::ota
        FX[钱包内换汇 UI]:::ota
        ORD[OTA 订单支付]:::ota
    end
...
> 三条充值线、一条钱包内换汇线、一条订单消费线。**法币充值这条线 EX 不参与**，由 OTA 平台自己对接第三方收单机构。

### 5.2 场景 A：C 端法币充值（EX 不参与）

**业务说明**：C 端用信用卡 / 本地钱包 / 银行转账充值。OTA 平台自己对接第三方收单机构（Stripe / 本地 PSP），收到法币后给 C 端钱包对应法币余额上账。

```mermaid
sequenceDiagram
    autonumber
    participant C as C 端旅客
    participant OTA as OTA 钱包
    participant PSP as 第三方收单机构<br/>(Stripe / 本地 PSP)
    participant Bank as OTA 法币结算账户

    C->>OTA: 选择"充值 100 USD"<br/>选支付方式（卡 / 本地钱包）
    OTA->>PSP: 创建支付订单
    PSP-->>C: 跳转 / SDK 收银台
    C->>PSP: 输入卡号 / 完成 3DS
    PSP->>Bank: 清算 USD 到 OTA 结算账户
    PSP->>OTA: Webhook：支付成功
    OTA->>OTA: C 端 USD 钱包 +100
    OTA-->>C: 充值成功
```

> **此流程不经过 EX**。OTA 平台选型 / 商务谈判 / 接入第三方 PSP 自行完成。

---

### 5.3 场景 B：C 端数币充值（EX 数币收银 + OffRamp 数转法）

**业务说明**：C 端选择数币充值 100 USD（法币）。OTA 平台调 EX 获取 OffRamp 报价 → 展示 "需转 X USDT" → C 端链上转账 → EX 自动触发数转法 → USD 到 OTA 平台在 SP 的 USD 账户 → OTA 平台给 C 端 USD 钱包加余额。

OTA 平台**不承担 FX 商角色**，汇率与换汇风险由持牌 SP 通过报价承担。
...
sequenceDiagram
    ...
    C->>OTA: 充值 100 USD（数币方式）
    OTA->>EX: POST /offramp/quote<br/>{from:USDT, to:USD, toAmount:100}
    EX->>SP: 询价 + 锁价
    SP-->>EX: rate + fromAmount
    EX-->>OTA: {quoteId, 100.X USDT, expiresAt}
    OTA-->>C: 展示 "需转 100.X USDT"

    C->>OTA: 确认
    OTA->>EX: POST /cashier/orders<br/>{bizOrderNo, quoteId, autoOffRamp:true}
    EX->>SP: 分配收款地址
    EX-->>OTA: {paymentInstructions: 链/地址/100.X USDT}
    OTA-->>C: 展示地址 + 二维码

    C->>SP: 链上转 USDT
    SP->>EX: 链上确认
    EX->>OTA: Webhook: CRYPTO_PAYIN_SUCCESS

    Note over EX,SP: autoOffRamp=true<br/>EX 自动触发数转法
    EX->>SP: 执行 OffRamp（按 quoteId）
    SP-->>SP: USD 入账 OTA 平台在 SP 的 USD 账户
    EX->>OTA: Webhook: OFFRAMP_SETTLED<br/>{bizOrderNo, fiatAmount:100 USD}

    OTA->>OTA: C 端 USD 钱包 +100
    OTA-->>C: 充值成功
```

> 也支持给 C 端 **USDT 余额** 直接充值（不做 OffRamp）：建单时不带 `quoteId / autoOffRamp`，到账后 OTA 平台直接给 C 端 USDT 钱包 +N。
...
| 模式                      | 说明                                                                                           | 适用               |
| ------------------------- | ---------------------------------------------------------------------------------------------- | ------------------ |
| **A. OTA 平台内部撮合** | OTA 平台内部账本直接换算（OTA 平台自己加点报价 / 自营 FX 池）；底层资金通过周期性批量调拨在 SP 端做 FX | 大量小额、频次高   |
...
sequenceDiagram
    ...
    C->>OTA: 钱包内：100 USD → EUR
    OTA->>EX: POST /fx/quote<br/>{from:USD, to:EUR, fromAmount:100}
    EX->>SP: 询价 + 锁价
    SP-->>EX: rate + toAmount
    EX-->>OTA: {quoteId, toAmount:9X EUR, expiresAt}
    OTA-->>C: 展示报价 + 倒计时
    C->>OTA: 确认

    OTA->>EX: POST /fx/convert {quoteId}
    EX->>SP: 执行 FX
    SP-->>SP: OTA USD 账户 -100<br/>OTA EUR 账户 +9X
    EX->>OTA: Webhook: FX_SETTLED

    OTA->>OTA: C 端 USD 钱包 -100<br/>C 端 EUR 钱包 +9X
    OTA-->>C: 兑换成功
```
...
**业务说明**：C 端在 OTA 平台下单（机票 / 酒店等），勾选 "用钱包余额支付"。**全程 OTA 平台内部账本扣减**，无外部资金动作。

```
1. C 端在 OTA 平台下单 → 选币种 + "钱包余额支付"
2. OTA 平台校验对应币种钱包余额
3. OTA 平台内部账本：C 端钱包 - 订单金额，OTA 平台应付供应商负债 + 订单金额
4. OTA 平台出票 / 确认订单
5. 后续 OTA 平台通过 VCC（可选）或法币付款向供应商结算
```

> 此场景不调 EX 任何接口；只是 OTA 平台内部账本动作。

---

### 5.6 场景 E：OTA 主体提现 / 提币

**业务说明**：OTA 平台把 SP 端的 USD / USDT 余额提到自己的银行账户或链上钱包。两者各自独立，无"归集"动作。
...
**业务说明**：保留原 OTA 场景。OTA 平台在 EX 申请 VCC，用 VCC 卡号在 GDS / 航司 / 酒店系统付款。

```
1. OTA 平台申请 VCC（机构卡，按订单 / 按月度发）
2. 从 OTA USD 账户充值卡账户
3. 在供应商系统输入 VCC 卡号 → 完成付款
4. EX 推送卡交易通知 → OTA 平台记录供应商结算完成
```
...
| OTA 主体 USD 提现                              | ✅ EX                | Payout                             | P0             |
| OTA 主体数币提币                               | ✅ EX                | 加密提现                           | P0             |
| VCC 向供应商结算                               | ✅ EX                | CARD_ISSUING                       | P1（OTA 沿用） |
| **C 端法币充值（卡 / 本地钱包 / 银行）** | ❌**OTA 平台自找** | Stripe / Adyen / dLocal / 本地 PSP | —             |
...
| VCC（可选） | VCC 申请 / 卡账户充值 / 查询 / 冻结解冻 / 限额                                             | 场景 F                                                                                 |
...
- 配置 OTA 商户在 EX 后台
- 固定 SP 路由（Crypto Checkout / OffRamp / FX / Payout 通道）
- 配置 Webhook URL 分发
- 联调高 TPS 压测（针对 C 端规模）
...
| **场景 E**：OTA 主体提现 / 提币                                          | `/payout/orders` + `/crypto/withdraw`                        | 3-5 天   | ✅              |
| **场景 D**：钱包余额支付订单（OTA 平台内部账本，无 EX 接口）                 | —                                                               | 3-5 天   | ✅              |
| **场景 F**：VCC 向供应商付款（可选 P1）                                  | VCC 卡申请 / 充值 / 实时授权                                     | 7-10 天  | ✅              |
| **场景 A**：C 端法币充值（OTA 平台自接第三方 PSP，**不计入 EX 工期**） | —                                                               | —       | 由 OTA 平台自行规划 |
