...
为商户申请法币支付产品。您只需提交商户材料，EX 将转发给持牌合规机构审核，结果通过 Webhook 通知。
...
- 审核期间可能触发 **RFI** 要求补充材料
- 可根据平台设计在用户注册、入网审核或首次使用法币支付功能时调用此流程

**产品类型：**

| 产品代码            | 说明     | 备注                                 |
| ------------------- | -------- | ------------------------------------ |
| `FIAT_COLLECTION` | 法币收款 | 包含 VA 账户、收款店铺管理、收款入账 |
| `FIAT_PAYOUT`     | 法币付款 | 包含收款人管理、单笔付款、结汇额度   |

**特殊流程说明：**

1. **收款类型区分**：当前区分外贸收款（跨境 B2B）和平台收款（电商平台）。如商户已有外贸收款需开通平台收款，请联系运营。预计 Q3 取消该区分。
2. **结汇至中国大陆**：外贸收款结汇至中国大陆的订单明细要求正在优化，预计 5 月底上线。



### 4.4 KYC 审核 — Know Your Customer
