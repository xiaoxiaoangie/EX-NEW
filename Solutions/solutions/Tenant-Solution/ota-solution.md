...
| **SP**                      | **实际的数币收单 / 承兑 / 法币换汇 / 付款持牌机构**。OTA 的法币 / 数币账户真实开在 SP 处                   |
...
> **法币充值通道由 OTA 平台自行接入**。EX 仅在数币侧（数币收单 + 数转法 OffRamp）和 OTA 主体的法币侧（FX / Payout）提供能力。
...
**业务说明**：保留原 OTA 场景。OTA 平台在 EX 申请虚拟卡，用 VCC 卡号在 GDS / 航司 / 酒店系统付款。

```
1. OTA 平台申请虚拟卡（机构卡，按订单 / 按月度发）
2. 从 OTA USD 账户充值卡账户
