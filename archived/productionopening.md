...
- ✅ **无系统路由**：商户下单时自行选择收款账户/支付来源，系统不做自动分配
- ❌ **不需要复杂条件判断**：不按金额/币种/国家等条件路由
- ❌ **不需要TP配置优先级**：本期无优先级概念，商户自行选择
...
✅ TP与SP签约（配置是否允许商户使用IPL法币账户）
✅ 商户注册（白牌Portal + API）
✅ 商户选择承兑产品（可选IPL法币账户）
✅ 入网推送：BB固定推送，IPL看商户是否勾选
✅ BB/IPL审核独立，各自通过各自可用
✅ 无系统路由：商户下单时自行选择收款账户/支付来源

本期不实现：
❌ 系统自动路由（按优先级/金额/币种/国家等条件）
❌ 商户级路由配置
...
**说明：** 本期不存在系统自动路由。商户在下单时自行选择，系统不做自动分配。

```
本期规则：

1. OnRamp 即时单（场景A）：商户自己选支付来源
   - 只有BB法币账户 → 只能选BB余额（场景A1）
   - 只有IPL法币账户 → 只能选IPL余额（场景A2）
   - BB+IPL都有 → 商户自己选从哪个法币账户扣款

2. OnRamp 预约单（场景B）：商户自己选收款账户
   - 只有BB → 展示BB代收付账户（场景B1）
   - 只有IPL → 展示IPL VA账户（场景B2）
   - BB+IPL都有 → 商户自己选用哪个收款账户

3. OffRamp 付款：资金在哪个SP账户就从哪出
   - BB法币账户余额 → 从BB出款
   - IPL法币账户余额 → 从IPL出款
   - 不存在跨SP路由选择
...
场景2：商户同时开通了BB和IPL
→ 商户下单时自己选择：
   - OnRamp即时单：选支付来源（BB余额 or IPL余额）
   - OnRamp预约单：选收款账户（BB代收付 or IPL VA）
   - OffRamp付款：资金在哪个SP账户就从哪出
```
...
sequenceDiagram
    ...
    rect rgb(255, 245, 240)
        ...
        MP->>EX: 15. 发起交易<br/>币种: USDT, 金额: 15000<br/>商户选择: BB法币账户余额
        EX->>SP: 16. 创建交易（按商户选择）
        SP->>EX: 17. 返回收款地址
        EX->>MP: 18. 返回收款信息
    end
...
✅ **多SP支持**：灵活配置入网推送规则
✅ **商户自主选择**：本期无系统路由，商户下单时自行选择收款账户/支付来源
✅ **职责清晰**：EX是科技平台，合规审核由SP负责

**关键流程：**

1. **前置准备**：SP上架产品 → TP签约 → TP配置是否允许商户用IPL
2. **商户入网**：注册 → 选择产品 → 提交资料 → SP审核 → 开通成功
3. **发起交易**：商户自行选择收款账户/支付来源 → 执行交易

---

*最后更新：2026-02-22*
*文档版本：v1.3 — 修正交易路由描述：本期无系统路由，商户自行选择*
