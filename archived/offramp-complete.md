...
7. [状态机](#7-状态机)（含商户端统一视图）
8. [计费与汇率](#8-计费与汇率)（→ `productfee.md`）
9. [风控规则与异常退款汇总](#9-风控规则与异常退款汇总)
10. [收款人管理](#10-收款人管理)（→ `vaandpayees.md`）
...
> **决策日期**：2026-02-26
> **完整决策背景详见** `onramp-complete.md` 第 3.1 章。

**OffRamp 与 OnRamp 共享同一决策：承兑业务一期由 BB 单一承接。**

- **承兑引擎**：BB（USDT/USDC → USD）
- **承兑法币账户**：BB 法币账户
...
| 场景 | 说明 |
| ---- | ---- |
| 纯承兑（场景 A） | 数币→法币入到 BB 法币账户，单一 SP 无歧义 |
| 带付款（场景 B） | 承兑后付款渠道可走 BB 自有通道或 IPL 法币通道（POBO），但承兑环节固定 BB |
| 多 SP 共存 | 承兑法币入 BB 账户，IPL 法币账户用于收付款业务，两个余额各有所属 |
...
> OffRamp 所有场景均为即时单，无预约单。商户在 MP 端看到的状态直接映射商户单状态：

| 商户单状态 | 商户端展示状态 | 商户可执行操作     | 说明                               |
| ---------- | -------------- | ------------------ | ---------------------------------- |
| CREATED    | Created        | 查看详情           | 订单已创建，等待系统处理           |
...
| REFUNDED   | Refunded       | 查看详情、查看退款详情 | 退款完成，资金已退回           |
...
| SUCCESS    | 成功       | 所有交易单完成                       |
| FAILED     | 失败       | 任一交易单失败                       |
| CANCELLED  | 已取消     | 商户主动取消（仅 CREATED 状态，未开始执行时可取消） |
...
> 收款人管理的详细规则见独立文档 `vaandpayees.md` 第4.3章，本文不再重复。
...
*最后更新：2026-02-23*
*文档版本：v1.1 — OffRamp 完整产品方案，4场景×先校验再建单，含异常处理和退款汇率规则；新增商户端统一视图、收款人管理章节；计费章节引用独立文档*
